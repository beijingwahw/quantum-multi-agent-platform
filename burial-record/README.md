...
- **85 batches / 644 errors**, every error in two columns — the wrong assertion as it stood, and the correction as recorded. The ledger's discipline (a number never travels without its cost) applied to ourselves: an error never travels without its fix. Batch 22 is this repo's own delivery log — the registry buries itself.
