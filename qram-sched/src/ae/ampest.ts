...
export function groverSuccessClosedForm(p: number, k: number): number {
  if (!(p >= 0 && p <= 1)) reject("AE_P_RANGE", "p in [0,1] for the Grover closed form");
  if (!Number.isInteger(k) || k < 0) reject("AE_K_RANGE", "k >= 0 Grover iterations");
...
export function groverFullSpace(q: number, marked: readonly number[], k: number): number {
  ...
  for (const m of marked) {
    ...
    if (seen.has(m)) reject("AE_FULLSPACE_PARAMS", `groverFullSpace: duplicate marked index ${m}`);
    seen.add(m);
  }
  if (!Number.isInteger(k) || k < 0) reject("AE_FULLSPACE_PARAMS", "groverFullSpace: k >= 0 iterations");
...
export function qaeDistribution(p: number, m: number): Float64Array {
  if (!(p > 0 && p < 1)) reject("AE_P_RANGE", "p in (0,1) for QAE (0 and 1 are read off trivially)");
  if (!Number.isInteger(m) || m < 1 || m > 30) reject("AE_M_RANGE", "m in [1,30] phase qubits");
  const M = 2 ** m;
  const theta = Math.asin(Math.sqrt(p));
...
export function mcMedianError(p: number, s: number, trials: number, seed: number): number {
  if (!(p >= 0 && p <= 1)) reject("AE_P_RANGE", "p in [0,1] for Monte Carlo");
