...
export function monotoneInstance(n: number): ObmInstance {
  if (!Number.isInteger(n) || n < 1 || n > 4096) reject("MATCH_ARG_RANGE", "n in [1,4096]");
  const arrivals: number[][] = [];
  for (let j = 0; j < n; j++) {
    const nb: number[] = [];
    for (let w = j; w < n; w++) nb.push(w);
    arrivals.push(nb);
  }
  return { n, arrivals };
}
...
export function sampleDnMember(n: number, rng: Rng): ObmInstance {
  // v0.3.0: this public entry previously had no input check at all.
  if (!Number.isInteger(n) || n < 1 || n > 4096) reject("MATCH_ARG_RANGE", "n in [1,4096]");
  const tau = rng.shuffle(Array.from({ length: n }, (_, i) => i));
  const arrivals: number[][] = [];
  for (let j = 0; j < n; j++) arrivals.push(tau.slice(j).sort((a, b) => a - b));
  return { n, arrivals };
}

/** d(m): derangement numbers, exact (d(0)=1, d(1)=0, d(m)=(m-1)(d(m-1)+d(m-2))). */
export function derangement(m: number): bigint {
  if (!Number.isInteger(m) || m < 0 || m > 5000) reject("MATCH_ARG_RANGE", "m in [0,5000]");
  if (m === 0) return 1n;
  let dPrev = 1n; // d(0)
  let dCur = 0n; // d(1)
  for (let i = 2; i <= m; i++) {
...
export function feigeRankingExpectation(n: number): number {
  if (!Number.isInteger(n) || n < 1 || n > 1024) reject("MATCH_ARG_RANGE", "n in [1,1024]");
 ...
 */
export function rankingExpectationExhaustive(n: number): number {
  if (!Number.isInteger(n) || n < 1 || n > 9) reject("MATCH_ARG_RANGE", "exhaustive enumeration limited to n <= 9");
  ...
  for (let i = 0; i < n; ) {
    if ((c[i] as number) < i) {
      const k = i % 2 === 0 ? 0 : (c[i] as number);
      const tmp = rank[i] as number;
      rank[i] = rank[k] as number;
      rank[k] = tmp;
      evaluate();
      c[i] = (c[i] as number) + 1;
      i = 0;
 ...
 */
export function greedyUniformExpectationExact(n: number): number {
  if (!Number.isInteger(n) || n < 1 || n > 24) reject("MATCH_ARG_RANGE", "subset DP limited to n <= 24");
  ...
  for (let j = 0; j < n; j++) {
    ...
    for (const [mask, prob] of states) {
      const avail: number[] = [];
      for (let b = 0; b < windowSize; b++) if (((mask >>> b) & 1) === 0) avail.push(b);
      if (avail.length === 0) {
        add(next, mask >>> 1, prob);
      } else {
        expected += prob;
        const p = prob / avail.length;
        for (const b of avail) add(next, (mask | (1 << b)) >>> 1, p);
      }
    }
    states = next;
  }
...
function add(map: Map<number, number>, key: number, delta: number): void {
 ...
 */
export function deterministicGreedyHalfInstance(n: number, rule: DeterministicTieBreak): ObmInstance {
  if (!Number.isInteger(n) || n < 2 || n % 2 !== 0) reject("MATCH_EVEN_N", "n must be even");
  const half = n / 2;
  const all = Array.from({ length: n }, (_, i) => i);
  const arrivals: number[][] = [];
  for (let i = 0; i < half; i++) arrivals.push([...all]);
  const taken = new Uint8Array(n);
  const matched: number[] = [];
  for (let i = 0; i < half; i++) {
    const avail = all.filter((w) => (taken[w] as number) === 0);
    const w = rule === "lowest" ? (avail[0] as number) : (avail[avail.length - 1] as number);
    taken[w] = 1;
    matched.push(w);
  }
  for (let i = 0; i < half; i++) arrivals.push([...matched]);
  return { n, arrivals };
}

// ---------------------------------------------------------------------------
// Certificate verifier: the tight-instance smuggling-trial checker.
// ---------------------------------------------------------------------------
...
const NAME_OBLIVIOUS: ReadonlySet<TightAlgorithm> = new Set(["ranking", "greedy-uniform"]);
 ...
 */
export function verifyTightCertificate(claim: TightClaim): TightVerdict {
  const violations: TightViolation[] = [];
  const { family, n, algorithm } = claim;
  let inst: ObmInstance;
  if (family === "kv-monotone") {
    ...
    if (algorithm === "ranking" || algorithm === "greedy-uniform") {
      violations.push({
        ...
        detail:
          "the deterministic half adversary certifies DETERMINISTIC rules (exact n/2); randomized rules must be " +
  ...
  let machineExpectedSize: number;
  if (family === "kv-monotone") {
    if (!NAME_OBLIVIOUS.has(algorithm)) {
      ...
      const reversed: ObmInstance = { n, arrivals: inst.arrivals.map((nb) => [...nb].reverse()) };
      const memberValue =
        algorithm === "greedy-lowest"
      ...
      violations.push({
        ...
        detail:
          `${algorithm} is name-aware: on the fixed MonotoneG member it matches ${memberValue} (ratio ` +
...
// ---------------------------------------------------------------------------
// Query-ledger verifier: the fake-ledger smuggling-trial checker.
// ---------------------------------------------------------------------------
 ...
 */
export function verifyQueryLedger(inst: ObmInstance, claim: LedgerClaim, seeds: number): LedgerVerdict {
  const violations: TightViolation[] = [];
  ...
  for (let s = 0; s < seeds; s++) {
    ...
    const res = rankingMatchWithRank(
      inst,
      rng.shuffle(Array.from({ length: inst.n }, (_, i) => i)),
      claim.mode,
      rng,
    );
    reads += res.reads;
