...
function main(): void {
  ...
  lines.push("");
  lines.push(
    "QAE is simulated exactly: m phase qubits, controlled-G^{2^j} on the 2-plane Grover rotation, exact inverse " +
  ...
  lines.push("");
  lines.push(
    "The ratio grows like ~1/eps: at accuracy 0.2% the quantum estimator uses ~32x fewer oracle calls; the " +
  ...
  let busP = 0;
  for (let a = 0; a < qram.numCells; a++) busP += (st.re[a * 2 + 1] as number) ** 2 + (st.im[a * 2 + 1] as number) ** 2;
  ...
  lines.push("");
  lines.push(
    "The encoded stream's bus-one probability IS the mean to machine precision: one query exposes the statistic, " +
