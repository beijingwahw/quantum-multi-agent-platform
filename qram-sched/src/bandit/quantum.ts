...
export function sampleQae(p: number, m: number, rng: Rng): number {
  ...
  for (let j = 0; j < dist.length; j++) {
    acc += dist[j] as number;
    if (u < acc) return qaeEstimate(j, m);
  }
 ...
 */
export function quantumReplayRun(
  ...
  for (let m = mStart; m <= mMax; m += mStep) {
    ...
    const gap = (sorted[0] as number) - (sorted[1] as number);
    committed = estimates.indexOf(sorted[0] as number);
    if (gap > (3 * Math.PI) / 2 ** m) {
      break;
    }
  }
  const regret = horizon * (best - (means[committed] as number));
