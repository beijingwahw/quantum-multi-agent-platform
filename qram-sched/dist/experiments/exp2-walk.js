 ...
 */
import { chainFromGraph, chainMatrix, lazyChain, SzegedyWalk, uniformAwayFrom } from "../walk/szegedy.js";
...
function assignmentLattice(t, w, seed) {
    ...
    const wel = states.map((d) => d.reduce((a, ass, i) => a + U[i]?.[ass], 0));
    let opt = -Infinity;
    for (const v of wel)
        if (v > opt)
            opt = v;
    ...
    for (let s = 0; s < n; s++)
        if (wel[s] >= opt - 0.005 * Math.abs(opt))
            marked.add(s);
...
function main() {
    ...
    lines.push("");
    lines.push("Walk operator per step: C . S . R (column diffusion, flip-flop swap, oracle flip on edges touching marked). " +
    ...
    lines.push("");
    lines.push(`Exponent fits: log2 QT vs log2 n slope = ${fmt(fitSlope(logN, logQT), 3)} (theory 0.5); ` +
    ...
    lines.push("");
    lines.push(`Exponent fit: log2 QT vs log2(1/q) slope = ${fmt(fitSlope(logQ, logQT2), 3)} (theory 0.5). ` +
    ...
    lines.push("");
    lines.push(`Marked set = welfare within 0.5% of OPT (a single near-optimal assignment in these instances). ` +
    ...
    lines.push("");
    lines.push("Two regimes, both reported: an early transient brushes the 0.25 threshold within a few steps (probabilities " +
