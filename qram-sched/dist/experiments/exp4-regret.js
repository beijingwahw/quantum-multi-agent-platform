...
export function main() {
    ...
    for (const T of [2500, 5000, 10000, 20000]) {
        ...
        for (let s = 0; s < seeds; s++) {
            ...
            if (u.decisions[T - 1] !== 0)
                finalWrong++;
            etc += etcRun(means, T, 300, s, "live").regret;
        }
        ucbRegrets.sort((a, b) => a - b);
        const median = ucbRegrets[Math.floor(seeds / 2)];
        rowsA.push([
            String(T),
            fmt(median, 1),
    ...
    lines.push("");
    lines.push("Medians (robust to the rare wrong-final-arm seed, counted separately): UCB1 regret grows on the ln(T) scale " +
    ...
    lines.push("");
    lines.push("Both replay schedulers sit at (near-)zero regret: moving from the live to the replay model is what removes " +
    ...
    for (let s = 0; s < 10; s++) {
        ...
        if (!identical || lin.regret !== quant.regret)
            bitIdentical = false;
    ...
    lines.push("");
    lines.push(`All 10 seeds: decisions bit-identical and regrets exactly equal (${bitIdentical ? "verified" : "VIOLATION — investigate"}) ` +
    ...
    lines.push("");
    lines.push("1. LIVE classical: exploration burns regret (ln T scale; Lai-Robbins ledger).\n" +
    ...
    lines.push("");
    lines.push("- The replay premise is strong: a qRAM holding a faithful simulator (or logs) of the environment. Live " +
