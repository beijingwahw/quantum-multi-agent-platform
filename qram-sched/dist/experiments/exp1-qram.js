...
export function main() {
    ...
    lines.push("");
    lines.push("Charging model (the bucket-brigade's OWN ledgers from sections A/B, in the spirit of Jaques-Rattew, " +
    ...
    for (const nb of [6, 10, 16, 20]) {
        ...
        for (const eps of [0.05, 0.01]) {
            ...
            const worstMedian = (m) => Math.max(...pBank.map((p) => qaeMedianError(p, m)));
            let m = 3;
            while (worstMedian(m) > eps)
                m++;
    ...
    lines.push("");
    lines.push("The honest hardware-metered reading: at eps = 0.01 one AE task buys ~3.6x-12x metered activations vs " +
    ...
    lines.push("- Section D's activation metering is bucket-brigade-specific (n_b activations per query/write) and ignores " +
