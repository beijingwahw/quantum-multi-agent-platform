 ...
 */
import { deterministicGreedyHalfInstance, feigeRankingExpectation, greedyUniformExpectationExact, monotoneInstance, rankingExpectationExhaustive, sampleDnMember, } from "../online/kv-tight.js";
...
export function main() {
    ...
    lines.push("");
    lines.push("Family (Feige, arXiv:1812.11774, Sec. 1.1, representing KVV STOC 1990's D_n): MonotoneG has arrival " +
    ...
    lines.push(table(["n", "E = a(n)/n! (Feige)", "ratio E/n", "E - (1-1/e)n", "|exhaustive - formula|", "|greedy DP - formula|"], rowsA));
    lines.push("");
    lines.push("Three independent exact computations agree: the published derangement formula, the n!-permutation RANKING " +
    ...
    lines.push(table(["algorithm", "fixed MonotoneG member (deterministic)", "D_n MC mean (K=2000)", "D_n exact"], [
    ...
    lines.push("");
    lines.push("The honest nuance: greedy-lowest matches PERFECTLY on the fixed member (each u_j takes v_j) and greedy-highest " +
    ...
    for (const n of [8, 16, 64, 128]) {
        for (const rule of ["lowest", "highest"]) {
    ...
    lines.push("");
    lines.push("The adversary (Feige Sec. 1 sketch): first n/2 arrivals see all workers; the rule's own phase-1 matches form " +
    ...
    lines.push("");
    lines.push(`On the tightest stage the separation restates itself: RANKING's mean ratio under the linear rule equals the ` +
    ...
    lines.push("");
    lines.push("- The exhaustive kernel stops at n <= 9 (362880 permutations) and the subset DP at n <= 24; beyond that the " +
...
function disagreementsNote(rows) {
    return rows.map((r) => `${r[0]}:${r[5]}`).join(", ");
}
