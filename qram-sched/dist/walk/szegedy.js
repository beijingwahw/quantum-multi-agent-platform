 ...
 */
function checkChainShape(c) {
    if (!Number.isInteger(c.n) || c.n < 1)
        reject("WALK_CHAIN_SHAPE", `n >= 1 states required (got ${c.n})`);
...
export function chainFromGraph(adj) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (nb.length === 0)
            reject("WALK_ISOLATED_VERTEX", `isolated vertex ${i}`);
...
export function uniformAwayFrom(n, targets) {
    ...
    if (free <= 0)
        reject("WALK_NO_TRANSIENT", "no transient states");
    ...
    constructor(chain, marked) {
        ...
        for (let e2 = 0; e2 < total; e2++) {
            ...
            if (!this.edgeIdx.has(y * this.chain.n + x))
                reject("WALK_ASYMMETRIC_SUPPORT", "chain support must be symmetric");
