...
export function ucb1Run(means, horizon, seed) {
    ...
    for (; t < Math.min(k, horizon); t++) {
        const a = t;
        const r = rng.bernoulli(means[a]) ? 1 : 0;
        pulls[a] = pulls[a] + 1;
        wins[a] = wins[a] + r;
        regret += (best - means[a]);
        decisions[t] = a;
    ...
    for (; t < horizon; t++) {
        ...
        const r = rng.bernoulli(means[arg]) ? 1 : 0;
        pulls[arg] = pulls[arg] + 1;
        wins[arg] = wins[arg] + r;
        regret += best - means[arg];
        decisions[t] = arg;
    }
...
export function adversarialRun(k, horizon, seed, gamma, search) {
    ...
    for (let i = 0; i < rewards.length; i++)
        rewards[i] = rng.bernoulli(0.5) ? 1 : 0;
