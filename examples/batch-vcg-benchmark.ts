 ...
 */
import { BatchVCGScheduler, BudgetPacer, type BatchAgentSpec } from '../src/core/batch-vcg-scheduler.js';
...
const AGENTS: BatchAgentSpec[] = [
  { id: 'a0', capabilities: CAPS,     trueCost: 1.0, capacity: 3, trueQuality: { A: 0.6, B: 0.6, C: 0.8 }, credentialQuality: { A: 0.6, B: 0.6, C: 0.8 } },
  { id: 'a1', capabilities: ['B'],    trueCost: 1.8, capacity: 3, trueQuality: { B: 0.85 },               credentialQuality: { B: 0.85 } },
  { id: 'a2', capabilities: CAPS,     trueCost: 2.6, capacity: 3, trueQuality: { A: 0.75, B: 0.5, C: 0.5 }, credentialQuality: { A: 0.75, B: 0.5, C: 0.5 } },
  { id: 'a3', capabilities: ['C'],    trueCost: 3.4, capacity: 2, trueQuality: { C: 0.8 },                credentialQuality: { C: 0.4 } },
  { id: 'a4', capabilities: ['A','C'],trueCost: 2.0, capacity: 2, trueQuality: { A: 0.7, C: 0.6 },        credentialQuality: { A: 0.55, C: 0.55 } },
];
...
console.log(`任务 ${TASKS.length} 个 | 无预算约束 VCG：W*=${slack.welfare} Σp=${vcgPayment.toFixed(2)} take=${slack.platformTake.toFixed(2)}`);
console.log(`短视基线：        W=${myopic.welfare} Σp=${myopic.totalPayment.toFixed(2)} take=${myopic.platformTake.toFixed(2)} 弃标=${myopic.droppedTasks}\n`);
...
for (const binding of [true, false]) {
  ...
  console.log(`  ${binding ? `预算紧(B=${(vcgPayment * 0.6).toFixed(1)})` : '预算松弛  '}：最大虚报收益 = ${worst.toFixed(4)} ${who}`);
}
...
for (const mu of [1.3, 1.8]) {
  ...
  let worst = 0;
  for (const agentId of Object.keys(alloc.payments)) {
    const g = s.measureMisreportGain(TASKS, agentId, [-0.3, -0.1, 0.1, 0.3, 0.5, 1.0], { affine: { mu } });
...
console.log(`  后半程平均支出 = ${lateMean.toFixed(2)}（预算 ${B.toFixed(2)}，偏差 ${((lateMean / B - 1) * 100).toFixed(1)}%）`);
console.log(`  平均真实福利 = ${welfareMean.toFixed(2)}`);

console.log('\n三方机制对比（同一预算 B）');
console.log('| 机制 | 预算保证 | DSIC | 支付水平 | 代价 |');
console.log('|---|---|---|---|---|');
console.log(`| λ-bisection | 每批硬约束 | 近似（实测 0） | ${(fresh().allocateBatch(TASKS, { budget: B }).totalPayment).toFixed(2)} | 乘子依赖当批报价 |`);
console.log(`| μ-VCG + Pacer | 平均守恒（后半程偏差 ${((lateMean / B - 1) * 100).toFixed(1)}%） | 精确 | ${lateMean.toFixed(2)} | 边际任务弃标 |`);
