...
async function main(): Promise<void> {
  ...
  if (token) {
    ...
    try {
      const result = await solveAssignmentOnBackend(problem, backend, { numReads: 200, timeoutMs: 120_000 });
      console.log(`  solver        : ${result.solver}`);
      console.log(`  assignment    : [${result.assignment}]`);
      console.log(`  welfare       : ${result.welfare.toFixed(3)} / 最优 ${result.optimality?.optimal.toFixed(3)}`);
      console.log(`  optimality    : ${(result.optimality ? result.optimality.ratio * 100 : 0).toFixed(1)}%`);
      console.log(`  采样频率      : ${(result.sampleFrequency * 100).toFixed(2)}% (${result.totalReads} reads)`);
      console.log(`  非法样本      : ${result.invalidSamples}/${result.totalReads}（真QPU噪声，已被闸门过滤）`);
    ...
    console.log(`      assignment = [${localResult.assignment}]  welfare = ${localResult.welfare.toFixed(3)}`);
    console.log(`      optimality = ${(localResult.optimality!.ratio * 100).toFixed(1)}%  （对照基准）`);

    // 2b. D-Wave 客户端编码验证（请求体格式检查，不发网络请求）
    const dwave = new DWaveBackend({ token: 'dry-run', endpoint: 'https://cloud.dwavesys.com/sapi/v2' });
    console.log(`\n  2b. D-Wave 客户端（${dwave.name}）:`);
    console.log(`      isAvailable = ${dwave.isAvailable()}（dry-run token）`);
    console.log('      提交格式: toIsing() 的 (h, J) → bqm 三元组（混合求解器）/ ising 字典（结构化 QPU）');
    ...
    console.log(`      换 IBM 真机: 将 AerSimulator() 替换为 qiskit_ibm_runtime 后端（文件内有 TODO 标注）`);
    ...
    for (let i = 0; i < 5; i++) {
      scheduler.registerAgent({
        id: `a${i}`, name: `Agent-${i}`, type: 'developer', capabilities: ['qpu-task'],
    ...
    for (let i = 0; i < 3; i++) {
      scheduler.submitTask({
        name: `量子任务-${i}`, type: 'qpu', priority: 'critical',
    ...
    const report = await scheduler.scheduleBatchQuantumQpu(new LocalQuantumBackend(), { numReads: 512 });
    ...
    for (const a of report.assignments) {
      console.log(`        ${a.taskName} → ${a.agentId} (采样频率 ${(a.probability * 100).toFixed(1)}%)`);
    }
