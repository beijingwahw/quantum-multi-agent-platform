 ...
 *
 * Platform-side copy of wukong-crossval/src/kernel/crossval.ts, synced with
 * the canonical's hardened surface (named refusals replace silent NaN /
 * wrapped-shift garbage on every degenerate boundary; the applyRX sign
 * convention is frozen and disclosed). Degenerate-input behavior MUST track
 * the canonical — the smoke gate pins the refusal codes so a re-drifted copy
 * fails loudly.
 */
...
export function quboValue(inst: QuboSpec, bits: number): number {
  if (inst.linear.length !== inst.n) {
    ...
    throw new XvalError(
      'XVAL_QUBO_SHAPE',
      `quboValue: linear must carry n=${String(inst.n)} coefficients, got ${String(inst.linear.length)} (coupling rows may be empty or trailing-omitted — boundary reads fall back to 0)`,
    );
  }
  let v = 0;
...
export function instanceSet(): Instance[] {
  ...
  out.push(
    finish(mkLinear(8, 101)),
    finish(mkLinear(12, 113)),
    finish(mkLinear(12, 777)),
    finish(mkLinear(16, 116)),
    finish(mkLinear(20, 120)),
  );
 ...
 * (the canonical docs/theory.md "the layout constant is shared by every
 * kernel function" is this accessor, not a repeated idiom). */
export function dimOf(psi: Float64Array): number {
  return psi.length >> 1;
}

/** |amplitude_k|^2 — the basis-state probability, single-sourced so the
 * expectation and the sampler share one expression (bit-identical to the
 * previously inlined re*re + im*im). */
...
export function requireStateLayout(psi: Float64Array, n: number, what: string): void {
  ...
  if (psi.length !== (1 << n) << 1) {
    throw new XvalError(
      ...
      `${what}: statevector must carry (1 << n) * 2 = ${String((1 << n) << 1)} entries for n=${String(n)}, got ${String(psi.length)}`,
    );
  }
}
...
export function applyCost(psi: Float64Array, costs: Float64Array, gamma: number): void {
  ...
  if (costs.length < dim) {
    throw new XvalError(
      'XVAL_LAYOUT_MISMATCH',
      `applyCost: cost table must cover every basis state (${String(dim)}), got ${String(costs.length)}`,
    );
  }
  for (let k = 0; k < dim; k++) {
    const c = costs[k]!;
 ...
 * SIGN CONVENTION (frozen, hand-anchored in the smoke gate): this applies
...
export function applyRX(psi: Float64Array, n: number, q: number, theta: number): void {
  ...
  if (!Number.isInteger(q) || q < 0 || q >= n) {
    throw new XvalError(
      'XVAL_N_RANGE',
      `applyRX: qubit index must be an integer in [0, n), got ${String(q)} for n=${String(n)}`,
    );
  }
...
function requirePairedParams(params: QaoaParams, what: string): void {
  if (params.betas.length !== params.gammas.length) {
    throw new XvalError(
      'XVAL_PARAMS_LENGTH',
      `${what}: betas and gammas must be paired per layer, got ${String(params.betas.length)} betas vs ${String(params.gammas.length)} gammas`,
    );
  }
}
...
export function expectation(psi: Float64Array, costs: Float64Array): number {
  ...
  if (costs.length < half) {
    throw new XvalError(
      'XVAL_LAYOUT_MISMATCH',
      `expectation: cost table must cover every basis state (${String(half)}), got ${String(costs.length)}`,
    );
  }
  let e = 0;
...
export function optimizeOffline(inst: Instance, p: number): QaoaParams {
  if (!Number.isInteger(p) || p < 0) {
    throw new XvalError(
      'XVAL_DEPTH_RANGE',
      `optimizeOffline: layer count must be an integer >= 0 (p=0 is the uniform-state anchor), got ${String(p)}`,
    );
  }
  ...
  const grid =
    tier === 'full'
      ? gridFull
      : tier === 'grid'
        ? [0, Math.PI / 4, Math.PI / 2, (3 * Math.PI) / 4, Math.PI, (3 * Math.PI) / 2]
        : [0, Math.PI / 4, Math.PI / 2, (3 * Math.PI) / 4, Math.PI];
  let best: QaoaParams = {
    betas: new Array<number>(p).fill(0),
    gammas: new Array<number>(p).fill(0),
  };
  let bestE = Infinity;
  const evalP = (params: QaoaParams): number => expectation(runQaoa(inst, params), costs);
  for (const g of grid) {
    for (const b of grid) {
      const params: QaoaParams = {
        betas: new Array<number>(p).fill(b / p),
        gammas: new Array<number>(p).fill(g / p),
      };
...
export function sampleWithReadoutNoise(
  ...
  if (!Number.isInteger(optBits) || optBits < 0 || optBits >= 1 << n) {
    throw new XvalError(
      'XVAL_BITS_RANGE',
      `sampleWithReadoutNoise: optBits must be a basis state in [0, ${String(1 << n)}) for n=${String(n)}, got ${String(optBits)}`,
    );
  }
  if (!Number.isInteger(shots) || shots < 1) {
    throw new XvalError(
      'XVAL_SHOTS_RANGE',
      `sampleWithReadoutNoise: shots must be an integer >= 1, got ${String(shots)}`,
    );
  }
