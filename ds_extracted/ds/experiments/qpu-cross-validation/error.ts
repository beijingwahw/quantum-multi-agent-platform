/**
 * The named error surface for the platform-side cross-validation kernel —
 * mirrored from the canonical wukong-crossval/src/kernel/error.ts, trimmed to
 * the codes this copy's kernel.ts consumes. Every kernel throw carries one of
 * these codes, and every public kernel entry rejects contraband input by name
...
export function requireQubitCount(n: number, what: string): void {
  if (!Number.isInteger(n) || n < 0 || n > 30) {
    throw new XvalError(
      'XVAL_N_RANGE',
      `${what}: qubit count must be an integer in [0,30] (1 << n wraps beyond 30), got ${String(n)}`,
    );
  }
}
