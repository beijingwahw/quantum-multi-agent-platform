...
> **v1.11 勘误**：该 0/5 是示例脚本给经典侧只记不含耦合加成半账的记账
> Artifact；统一记账下经典侧贪心/局部搜索各 2/5（量子 5/5 与线性逐点一致
> 不变），详见 README 基准节勘误。
...
> 结构快照写于 v1.0，目录与文件现状以 README.md「项目结构」为准（测试套件现为 104 文件 / 1171 用例，文档与性能子目录见仓库实况）。
...
（来源命令 `npm run performance`，可复现；绝对吞吐随机器与热状态浮动，相对优化倍数是稳定口径）
```
...
- **测试**：1171 个用例 / 352 个套件（`npm test`：0 失败 /
  性能灵敏度用例按测量环境守卫自跳过，跳过数随机器负载浮动）
