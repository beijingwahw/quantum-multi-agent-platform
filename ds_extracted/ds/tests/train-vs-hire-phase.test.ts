 ...
 *
 * 05#13 统计功效注记：相图断言每格 12 个固定种子（下方 SEEDS）。
 * diffRate 是蒙特卡洛差分口径，每种子 T=150 期——格间断言用 ±2pp
 * 显著性门限 + ±4pp 幅度容差吸收 12 种子的抽样噪声（每格增加种子的
 * 边际收益递减，18 格 × 12 种子的运行预算已使理论/经验符号一致在
 * 全部显著格子稳定成立）。需要更高功效的坍缩/K_min 定位断言使用
 * 下方的 24 种子扩展集（seeds24，「坍缩与 K_min 的经验验证」组）。
 */
