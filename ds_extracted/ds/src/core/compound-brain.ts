...
/** fitGrid 的返回形状（缓存条目的载荷，见 fitGridCached） */
...
interface CapabilityState {
  ...
  /**
   * 观测变更计数（08#3）：append 与 OBS_CAP 容量淘汰都递增——fitGrid 缓存
   * 的键。不能用数组长度：淘汰区长度恒定而内容在换（同长度不同内容），
   * 按长度缓存会在淘汰区静默返回过期拟合。
   */
  observationsVersion: number;
  /**
   * fitGrid 精确 memo（08#3）：观测未变 → 复用上次结果（缓存值即首次
   * 计算的同一组 double，bit-identical）；变了 → 重算并覆盖。
   */
  fitCache: { version: number; result: FitGridResult } | null;
...
export class CompoundBrain extends EventEmitter {
   ...
   */
  private fitGrid(obs: Array<{ base: number; k: number; success: boolean }>): FitGridResult {
  ...
  /**
   * fitGrid 的精确 memo（08#3「经常性 CPU 税」收口）：51×41 网格 × 观测数
   * 的对数似然全量重算（≈百万次对数求值）只在观测真正变更后发生。
   * 键 = 观测变更计数（observationsVersion，append 与容量淘汰都递增）；
   * 命中时返回缓存的原对象——值是首次计算的同一组 double，位级不变
   * （mixture 的权重归一在 fitGrid 返回前完成，此后只读；跨命中复用
   * 同一引用并再次赋给 cs.mixture，数值语义与重算完全相同）。
   * 缓存生命周期 = 该能力的整个学习史（无跨批价问题：观测是唯一的输入，
   * 与 EdgeMemo 的「严禁跨 allocateBatch」约束不同源）。
   */
  private fitGridCached(cs: CapabilityState): FitGridResult {
    if (cs.fitCache?.version === cs.observationsVersion) {
      return cs.fitCache.result;
    }
    const result = this.fitGrid(cs.observations);
    cs.fitCache = { version: cs.observationsVersion, result };
    return result;
  }
  ...
  private calibrate(c: string): void {
    ...
    const { best, llNull, mixture } = this.fitGridCached(cs);
  ...
  settle(taskId: string, success: boolean): boolean {
    ...
    // 观测变更计数（08#3）：本结算内的 append 与（若达上限的）容量淘汰
    // 都已落盘——一次递增同时覆盖两类变更，fitGrid 缓存由此失效
    cs.observationsVersion++;
