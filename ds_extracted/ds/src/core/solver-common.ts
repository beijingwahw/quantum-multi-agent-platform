...
export function minMaxOf(values: Readonly<Float64Array>): { min: number; max: number } {
  let min = Infinity;
  let max = -Infinity;
  // 索引循环遍历 TypedArray（01#18）：for..of 走迭代器协议，每次取值一次
  // 协议调用——能量谱量程计算在每次退火求解都跑，纯开销，索引循环语义
  // 严格相同（同元素序、同比较链）
  // eslint-disable-next-line @typescript-eslint/prefer-for-of -- TypedArray 热路径：迭代器协议是每次取值的纯开销
  ...
  for (let r = 0; r < restarts; r++) {
    ...
    while (delta > ANGLE_STEP_MIN) {
      ...
      for (let i = 0; i < angles.length; i++) {
        ...
        // 探针写法注记（01#18 实测结论）：[1,-1] 字面量与 candidate slice
        // 副本看似热路径分配噪音，实则被 V8 逃逸分析完全消除——字面量
        // 不逃逸可被标量替换，evaluate 只收到独立副本还保住了「angles
        // 未被回调改写」的别名信息。交错 A/B 基准（同轨迹同评估数）实测：
        // 字面量提升为模块常量反而 ~2x 慢、原位探针 + 失败回写 ~1.3x 慢，
        // 故保留本写法（评估数逐位相同的三变体对照见审计记录）
  ...
  for (let r = 0; r < restarts; r++) {
    ...
    while (delta > ANGLE_STEP_MIN) {
      ...
      for (let i = 0; i < angles.length; i++) {
        ...
        // 探针写法注记：同层版实现（见上）——字面量与 slice 副本经 V8
        // 逃逸分析优化后优于常量提升/原位探针，保留原写法
        for (const sign of [1, -1]) {
          const candidate = angles.slice();
          candidate[i] = Math.min(bound, Math.max(0, candidate[i]! + sign * delta));
          const value = evaluationsOf(candidate);
 ...
 * 累积边界上时不被采中）。
 *
 * cum 分配的定案注记（01#18 派生，实测 2026-09-06）：每次调用分配一个
 * dim 维 Float64Array（dim=604800 时约 4.8MB），实测分配+CDF 构建
 * 1.45ms/次——对照同维度一遍全量扫描 1.15ms，而该维度一次求解要跑
 * 数十步演化（百毫秒~秒级），cum 成本占比 <1%。本函数每次求解仅调用
 * 一次（非每次评估），跨调用复用需改导出签名并在调用链穿透暂存缓冲，
 * 收益不抵 API 侵蚀——刻意不优化，改动前先重测本注记的量级前提。
 */
