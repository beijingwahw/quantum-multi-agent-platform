...
function expectationOfSubspace(state: SubspaceState, energies: Float64Array): number {
  // expectationValueInto（01#18）：|ψ|² 逐位内联进累加，省一次 dim 维
  // Float64Array 分配；运算次序与 expectationValue 逐位一致（见其文档）
...
function collapseSubspace(
  ...
  let mass = 0;
  // 索引循环遍历 TypedArray（01#18）：坍缩每次求解都做全量求和，
  // 迭代器协议是每次取值的纯开销；同元素序累加，逐位不变
  // eslint-disable-next-line @typescript-eslint/prefer-for-of -- TypedArray 求和热路径：索引循环免迭代器协议开销
...
export function serialAnnealEvolve(
  ...
  const k = prepareSerialEvolution(model, energies, tau, steps);
  for (let t = 1; t <= steps; t++) {
    applyEvolutionStep(k, t / steps);
  }
  return { re: k.state.re, im: k.state.im };
}

/**
 * 串行绝热退火的非阻塞变体：算符序列与串行版共享单源（prepare +
 * applyEvolutionStep，数值逐位一致由测试锚定），唯一区别是按忙时预算
 * 让出事件循环——连续占用超过 SERIAL_YIELD_BUDGET_MS（Date.now()
 * 粗粒度即可，纳秒级开销）时 await 一次 setImmediate，使 WS/HTTP 心跳、
 * GC、immediate 队列在无 Worker 环境（QUANTUM_DISABLE_PARALLEL=1 /
 * SAB 不可用回退串行）下仍然存活。小维度快演化几乎不触发让出（开销趋
 * 零），大维度每步毫秒级时近似每步让出——annealSolveSubspaceAsync 的
 * 回退路径据此兑现「异步入口不冻结事件循环」的完整契约。
 */
export async function serialAnnealEvolveAsync(
  model: SubspaceModel,
  energies: Float64Array,
  tau: number,
  steps: number,
): Promise<{ re: Float64Array; im: Float64Array }> {
  const k = prepareSerialEvolution(model, energies, tau, steps);
  let yieldAt = Date.now() + SERIAL_YIELD_BUDGET_MS;
  for (let t = 1; t <= steps; t++) {
    applyEvolutionStep(k, t / steps);
    if (Date.now() >= yieldAt) {
      yieldAt = Date.now() + SERIAL_YIELD_BUDGET_MS;
...
/** 串行演化就绪态：初态、递推相位核（两条驱动路径的单源，防双实现漂移） */
interface SerialEvolutionKernel {
  state: SubspaceState;
  mixers: SubspaceModel['mixers'];
  dt: number;
  phRe: Float64Array;
  phIm: Float64Array;
  zRe: Float64Array;
...
function prepareSerialEvolution(
  model: SubspaceModel,
  energies: Float64Array,
  tau: number,
  steps: number,
): SerialEvolutionKernel {
  ...
  return { state, mixers: model.mixers, dt, phRe, phIm, zRe, zIm, dim };
}

/**
 * H(s) = −(1−s)·ΣA + s·C 的单步算符（同步/异步驱动共享；均匀初态是
 * −ΣA 的基态——Perron-Frobenius）。
 */
function applyEvolutionStep(k: SerialEvolutionKernel, s: number): void {
  for (const group of k.mixers) {
    k.state.applyFiberMixer(group, (1 - s) * k.dt, -1); // −A 方向（基态支路）
  }
 ...
 * 主线程事件循环全程存活（HTTP/WS 心跳、GC 不停摆）；并行不可用回退
 * 串行时同样经 serialAnnealEvolveAsync 按忙时预算让出——异步入口在
 * 任何环境配置下都不冻结事件循环。坍缩与报告收尾与同步路径共用
 * finishAnnealSolution——同一 dispatch 序列保证与同步求解器逐位一致
 * （由测试锚定）。
 */
