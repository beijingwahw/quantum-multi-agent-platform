...
import type { ExecutionTierRouting } from './execution-tier.js';
...
export interface QpuSolveOptions {
  ...
  timeoutMs?: number;
  /**
   * 执行层级路由（可选）：提交前由 FTQC 估算器判定 NISQ 直发 /
   * FTQC 排队（抛 FtqcDeferredError，画像随行）/ 经典回退（转本地精确引擎）。
   * 省略时行为与历史版本完全一致。
   */
  routing?: ExecutionTierRouting;
}
