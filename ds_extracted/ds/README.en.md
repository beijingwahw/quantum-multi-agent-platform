...
npm test                 # 1297 tests · 0 failures
...
**1297 tests · 0 failures · 113 files / 397 suites**; six gates green; coverage ratchet 92/82/92/92 with measured 94.2% statements / 86.2% branches; knip dead-code sweep clean; single runtime dependency (`ws`).
...
2. Subspace dimension still grows combinatorially as P(n,m): the scheduler batch path defaults to 2²⁰ (SCHEDULER_SUBSPACE_CAP, ~150 MB, overridable via `config.subspaceCap`) while the engine-wide default is 2²¹ (SUBSPACE_DIMENSION_CAP); the 8×10 case is a **decision-quality mode**, not the hot path.
