...
plt.rcParams.update({
    ...
    "grid.color": GRID,
    "grid.linewidth": 0.6,
    "axes.axisbelow": True,   # 网格压在柱体下方，不再横穿柱面
    "font.size": 11,
})

# 边框色 → 同族浅底：全部框体走这套（杜绝随手杂色）
TINT = {INDIGO: "#E8EAFB", CYAN: "#E4F6FA", GREEN: "#E9F9F1", AMBER: "#FFF4E0",
        RED: "#FBE9E7", GRAY: "#F0F1F5", DEEP: "#EDEEF3"}

VER = "R10"
OVERFLOWS = []  # (figure-title, text-head, text_w/box_w) —— 框内文本溢出自检记录
COLLIDES = []   # (figure-title, kind, desc-a, desc-b) —— 文本/框碰撞与越界自检记录
PENDING = []    # (fig, text-artist, patch) —— save() 时按最终布局测量
BOXES = []      # (fig, patch, label) —— 框-框碰撞 / 越界检查登记
...
def _contained(inner, outer, tol=0.94):
    """inner 几乎完全落在 outer 内（分层底板 vs 内嵌框属合法包含）"""
    return (_inter(inner, outer) >= tol * inner.width * inner.height
            and inner.x0 >= outer.x0 - 2 and inner.x1 <= outer.x1 + 2
...
def save(fig, name):
    ...
    # 视觉自检：以最终渲染尺寸测量（savefig 后布局已定，重测一次）
    fig.canvas.draw()
    ...
    # 碰撞门禁一：文本-文本两两相交 >18% 小面积即记（标题/脚注/标注一视同仁）
    texts = [(t, t.get_window_extent(renderer)) for ax in fig.axes for t in ax.texts if t.get_text().strip()]
    texts += [(t, t.get_window_extent(renderer)) for t in fig.texts if t.get_text().strip()]
    for i in range(len(texts)):
        for j in range(i + 1, len(texts)):
            ta, ea = texts[i]
            tb_, eb = texts[j]
            inter = _inter(ea, eb)
            small = min(ea.width * ea.height, eb.width * eb.height)
            if small > 0 and inter > 0.18 * small:
                COLLIDES.append((title, "text/text", ta.get_text().split("\n")[0][:14], tb_.get_text().split("\n")[0][:14]))

    # 碰撞门禁二：框-框部分重叠（互不包含才算）+ 越出坐标区（01 号塌画布的元凶类）
    fb = [(p, p.get_window_extent(renderer), lab) for f, p, lab in BOXES if f is fig]
    for i in range(len(fb)):
        ...
        for j in range(i + 1, len(fb)):
            ...
            inter = _inter(ea, eb)
            small = min(ea.width * ea.height, eb.width * eb.height)
            if small > 0 and inter > 0.04 * small and not (_contained(ea, eb) or _contained(eb, ea)):
                COLLIDES.append((title, "box/box", la, lb))
        ax = pa.axes
        if ax is not None:
            ab = ax.get_window_extent(renderer)
            if ea.x0 < ab.x0 - 0.05 * ab.width or ea.x1 > ab.x1 + 0.05 * ab.width:
                COLLIDES.append((title, "box/out-of-axes", la, f"x=[{ea.x0:.0f},{ea.x1:.0f}] axes=[{ab.x0:.0f},{ab.x1:.0f}]"))
    for t, e in texts:
        ax = getattr(t, "axes", None)
        if ax is not None and t.get_clip_on() is not True:
            ab = ax.get_window_extent(renderer)
            if e.x0 < ab.x0 - 0.06 * ab.width or e.x1 > ab.x1 + 0.06 * ab.width:
                COLLIDES.append((title, "text/out-of-axes", t.get_text().split("\n")[0][:14], ""))
    BOXES[:] = [b for b in BOXES if b[0] is not fig]
    plt.close(fig)
...
def box(ax, x, y, w, h, text, fc=LIGHT, ec=INDIGO, tc=DEEP, fs=10.5, lw=1.4, style="round,pad=0.02,rounding_size=0.025", weight="normal", zorder=3):
    ...
    BOXES.append((ax.figure, patch, text.split("\n")[0][:14]))
    return patch
...
def cap_ax(ax, y, text):
    """统一脚注：坐标区下方居中灰字"""
    return ax.text(0.5, y, text, transform=ax.transAxes, ha="center", va="top", fontsize=8.8, color=GRAY, linespacing=1.5)
...
def fig_architecture():
    ...
    def row(y, label, items, bg, boxfc="white"):
        """items: list of (text, width_ratio)；宽度占比归一化后等距排布，返回 [(cx, x, w)]"""
        ...
        ratios = [wr for _, wr in items]
        placed = []
        x = L + 0.2
        for (text, wr), r in zip(items, ratios):
            w = total_w * r / sum(ratios)
    ...
    # 层间主干连接：中央泳道一条清晰自上而下的调用链
    spine_x = 6.0
    for y_top, y_bot in ((8.73, 8.41), (6.63, 6.31), (4.43, 3.51)):
        arrow(ax, spine_x, y_top, spine_x, y_bot, color=INDIGO, lw=2.0, ms=16)
    # 引擎 2 ↔ 引擎 3 最优率对照（短箭头卡框间缝，标签引到带外下方）
    cmp_x = l1[1][1] + l1[1][2] + 0.14
    arrow(ax, cmp_x + 0.02, 2.31, l1[2][1] - 0.02, 2.31, color=CYAN, lw=1.8, ls="--", style="<|-|>", ms=11, zorder=4)
    ax.plot([cmp_x, cmp_x], [2.18, 1.50], color=CYAN, lw=1.0, ls=":", zorder=2)
    ax.text(cmp_x, 1.40, "最优率对照", fontsize=8.8, color=CYAN, ha="center", va="top", zorder=4)
    ax.text(R - 0.05, 0.42, "研究线（CompoundBrain · 批量VCG · 增长市场 · 主动智能）经 MarketBrain 接口接入",
...
def fig_pipeline():
    ...
    for i, (t, fc, ec) in enumerate(steps):
        ...
        if i:
            arrow(ax, x - gap + 0.03, 2.95, x - 0.03, 2.95, color=GRAY, ms=15)
    ax.text(7.0, 4.55, "调度器视角：每一批都是一次「编码 → 演化 → 观测」的完整量子过程", fontsize=11.5, ha="center", color=INDIGO, fontweight="bold")
    ax.text(7.0, 1.30, "关键点：约束不靠罚项——合法分配集合本身就是希尔伯特空间的基；\n演化算符在纤维上有闭式解（O(dim) 精确施加，零 Trotter 误差）",
            fontsize=9.5, ha="center", va="top", color=GRAY, linespacing=1.6)
...
def fig_hamiltonian():
    ...
    fig.suptitle("调度问题 → 物理问题：哈密顿量编码（示例实例 6任务×8agent，种子 7）", fontsize=15, fontweight="bold")
    ...
    a1.set_title("亲和度矩阵 w（优先级×能力匹配）", fontsize=11.5)
    a1.grid(False)
    for i in range(m):
        for j in range(n):
            a1.text(j, i, f"{w[i, j]:.2f}", ha="center", va="center", fontsize=7.4, color="white" if w[i, j] > 0.68 else DEEP)
    cb = fig.colorbar(im, ax=a1, fraction=0.045)
    cb.set_label("亲和度 w", fontsize=9, color=GRAY)
    cb.ax.tick_params(labelsize=8)

    a2.set_xlim(0, 10); a2.set_ylim(0, 10); clean(a2)
    a2.set_title("纠缠 = 物理耦合", fontsize=11.5)
    ...
    a2.text(5.5, 1.30, "J<0 冲突惩罚", fontsize=9.5, color=RED, ha="center")
    a2.text(5.0, 5.6, "耦合项进入哈密顿量\n可实测翻转联合最优解\n（测试钉住的对照面）", fontsize=9, ha="center", va="top", color=GRAY, linespacing=1.6)

    a3.set_xlim(0, 10); a3.set_ylim(0, 10); clean(a3)
    a3.set_title("代价哈密顿量 C（对角编码）", fontsize=11.5)
    ...
    a3.text(5, 6.1, "基态 = 最优联合分配\n（能量取负福利：E(x) = −W(x)）", fontsize=10, ha="center", color=DEEP, linespacing=1.6)
...
def fig_subspace():
    ...
    ax.set_yscale("log")
    ax.set_ylim(1e2, 3e25)
    ax.set_xticks(x, cases)
    ax.set_ylabel("希尔伯特空间维度（对数刻度）")
    ax.set_title("为什么「等效 80 量子比特」是可能的：约束即基", fontsize=15, fontweight="bold", pad=12)
    for i, (f, s) in enumerate(zip(full, sub)):
        if i == 3:  # 8×10：右端两根柱直接带结论小注，替代原长箭头标注
            ax.text(i - 0.2, f * 1.7, "$2^{80}$\n≈ 10¹⁵ TB（不可行）", ha="center", fontsize=8.6, color=RED, linespacing=1.4)
            ax.text(i + 0.2, s * 1.9, "1,814,400\n150 MB · 42 秒", ha="center", fontsize=8.4, color=INDIGO, linespacing=1.4)
        else:
            ax.text(i - 0.2, f * 1.7, f"$2^{{{exps[i]}}}$", ha="center", fontsize=10, color=RED)
            ax.text(i + 0.2, s * 1.7, f"{s:,.0f}", ha="center", fontsize=8.6, color=INDIGO, rotation=0)
    ax.legend(loc="upper left", frameon=False)
    cap_ax(ax, -0.13, "P(n,m) = n!/(n−m)!：每任务占一个不同 agent 的所有排列——「每行恰一个 1」的约束直接长在基上，罚项为零。")
...
def fig_fiber():
    ...
    ax.set_xlim(0, 12.45); ax.set_ylim(0, 8.6)
    clean(ax)
    ax.set_title("纤维混合器：约束子空间上的闭式幺正（示意：5 agent · 3 任务）", fontsize=15, fontweight="bold", pad=12)

    tasks = {"T1": (2.0, 7.5), "T2": (5.4, 7.5), "T3": (8.8, 7.5)}
    agents = {"A2": (1.2, 3.4), "A4": (4.2, 3.4), "A1": (7.0, 3.4), "A3": (9.4, 3.4), "A5": (11.6, 3.4)}
    ...
    for k, (x, y) in agents.items():
        fixed = k in ("A2", "A4")   # T2→A2、T3→A4 固定；T1 的自由落点 = 其余 agent
    ...
    # 纤维固定部分：T2→A2、T3→A4（橙色实线）
    arrow(ax, 5.4, 7.05, 1.25, 3.9, color=AMBER, lw=2.4)
    arrow(ax, 8.8, 7.05, 4.25, 3.9, color=AMBER, lw=2.4)
    ax.text(10.0, 6.7, "纤维固定部分\nT2→A2 · T3→A4", fontsize=9.8, color=AMBER, ha="left", va="top", linespacing=1.5)

    # T1 的三个自由落点（靛蓝虚线）：A1 · A3 · A5
    for tx, ty in ((7.0, 3.4), (9.4, 3.4), (11.6, 3.4)):
        arrow(ax, 2.0, 7.05, tx - 0.05, 3.95, color=INDIGO, lw=1.5, ls="--")
    ax.text(0.35, 5.9, "T1 的自由落点\n（其余任务固定）", fontsize=9.8, color=INDIGO, ha="left", va="top", linespacing=1.5)

    # 完全图 K3 = {A1, A3, A5}（底部）
    k3 = [(7.0, 9.4), (9.4, 11.6), (7.0, 11.6)]
    for x1, x2 in k3:
        arrow(ax, x1, 2.25, x2, 2.25, color=INDIGO, lw=1.8, style="<|-|>", ms=10)
    ax.text(9.3, 1.9, "完全图 $K_3$", fontsize=10.5, color=INDIGO, fontweight="bold", ha="center")

    box(ax, 0.35, 0.25, 11.75, 1.30,
...
def fig_qaoa():
    ...
    fig.suptitle("QAOA 变分电路与两种角度布局（layer / ma-QAOA）", fontsize=15, fontweight="bold")

    a = a1
    a.set_xlim(0, 13); a.set_ylim(0, 8.9); clean(a)
    ...
    a.text(6.5, 8.35, "$|s\\rangle$ = 均匀叠加（或纤维初态）", fontsize=9.5, ha="center", color=GRAY)
    box(a, 1.7, 1.4, 2.25, 6.3, "代价层\n$U_C(\\gamma)$\n\n$e^{-i\\gamma C}$\n哈密顿量编码", fc=TINT[INDIGO], ec=INDIGO, fs=8.8)
    box(a, 4.25, 1.4, 2.25, 6.3, "混合层\n$U_M(\\beta)$\n\n纤维旋转/X 旋转\n探索合法解", fc=TINT[CYAN], ec=CYAN, fs=7.8)
    box(a, 6.8, 1.4, 2.25, 6.3, "代价层\n$U_C(\\gamma_2)$", fc=TINT[INDIGO], ec=INDIGO, fs=8.8)
    box(a, 9.35, 1.4, 2.25, 6.3, "混合层\n$U_M(\\beta_2)$", fc=TINT[CYAN], ec=CYAN, fs=7.8)
    box(a, 11.9, 1.4, 0.95, 6.3, "测量\n$\\langle E\\rangle$", fc=TINT[GREEN], ec=GREEN, fs=8.6)
...
def fig_annealing():
    ...
    fig.suptitle("绝热退火：慢过临界点，基态全程跟随（能级为示意图）", fontsize=15, fontweight="bold")
    ...
    a1.annotate("最小 gap（临界点）\n步长在此最细", xy=(s[imin], (e0[imin] + e1[imin]) / 2), xytext=(0.22, 0.32),
...
def fig_born():
    ...
    fig.suptitle("Born 规则坍缩：概率就是末态振幅的模方（12 基态玩具示例，示意）", fontsize=15, fontweight="bold")
    ...
    order_desc = np.argsort(prob)[::-1]      # 概率降序：第一名在前
    for rank, idx in enumerate(order_desc[:3], start=1):
        a2.text(idx, prob[idx] + 0.004, f"top-{rank}", ha="center", fontsize=8.6, color=GREEN, fontweight="bold")
    a2.set_xticks(xs, labels, fontsize=8.5)
    a2.set_ylabel("概率 P(x) = |ψ(x)|²")
    a2.set_title("观测量子概率 → 决策读出（top-K 候选）", fontsize=11.5)
    fig.text(0.53, 0.012, "调度器的 decision.probability 是真实量子概率，不是分数归一化；top-K 用线性选择（O(dim)）取代全量排序。",
...
def fig_ladder():
    ...
    ax.set_title("子空间规模阶梯：等效 30→80 量子比特，最优率全程 100%（实测）", fontsize=15, fontweight="bold", pad=12)
    ax2 = ax.twinx()
    ...
    cap_ax(ax, -0.21, "贪心差距 14.1% / 16.5% / 5.2% / 6.7%；退火最优率四档全部 100.0%（穷举裁判）。\n时间口径：同机同状态实测，绝对值随热状态浮动。")
...
def fig_linear():
    ...
    fig.suptitle("线性赛道（25 实例，真实 bench 报告）：量子 × 匈牙利 逐点一致（独立算法互证）", fontsize=15, fontweight="bold")
    allv = []
    for r in lin:
    ...
    for ax, (solver, label, color) in zip(axes, solvers):
        ...
        ax.annotate("y = x", xy=(2.95, 2.30), fontsize=8.5, color=GRAY, ha="center")
...
def fig_nphard():
    ...
    order = [("greedy", "贪心"), ("greedy+local-search", "局部搜索"), ("simulated-annealing", "模拟退火"),
             ("hungarian", "匈牙利\n(耦合外行)"), ("subspace-exact", "量子\n子空间")]
    hits = [sum(1 for r in sub if r["solver"] == s and r["hit"]) for s, _ in order]
    labels = [l for _, l in order]
    fig, (a1, a2) = plt.subplots(1, 2, figsize=(12.0, 4.9), gridspec_kw={"width_ratios": [1.05, 1]})
    fig.suptitle("NP-hard 耦合赛道（6×8 × 5 公开种子，统一记账，真实 bench 报告）", fontsize=15, fontweight="bold")
    ...
    a1.tick_params(axis="x", labelsize=8.8)
    ...
    a2.set_xticks(range(1, 6), labs, fontsize=8.8)
    a2.set_ylabel("福利差距 gap（对最优）")
    a2.set_title("差距分布：量子全部贴 0", fontsize=11.5)
    a2.text(5, 0.004, "5/5 种子\ngap 全为 0", ha="center", va="bottom", fontsize=8.4, color=INDIGO, fontweight="bold", linespacing=1.4)
...
def fig_parallel():
    ...
    ax.set_title("v1.6 确定性并行演化：8×10 实例（1,814,400 维）同机同状态实测", fontsize=15, fontweight="bold", pad=14)
    mult = [1, "5.1×", "23.5×"]
    ...
    cap_ax(ax, -0.155, "并行＝确定性：worker_threads + SharedArrayBuffer + Atomics 屏障，每条纤维完整归属单线程，\n与串行路径共享同一份内核源码——结果逐位一致（测试逐位断言钉死）")
...
def fig_ma_qaoa():
    ...
    for i in range(2):
        ...
        ax.annotate("", xy=(i + 0.18, multi[i] + 1.5), xytext=(i - 0.18, layer[i] + 1.5),
                    arrowprops=dict(arrowstyle="-|>", color=GREEN, lw=1.6, connectionstyle="arc3,rad=0.3"))
        ax.text(i, (layer[i] + multi[i]) / 2 + 2.6, f"+{multi[i] - layer[i]}", ha="center",
                fontsize=9.5, color=GREEN, fontweight="bold",
                bbox=dict(fc="white", alpha=0.85, ec="none", pad=1.0))
    ax.set_xticks(x, ["p = 1", "p = 2"])
    ax.set_ylim(0, 44)
    ax.set_xlim(-0.58, 1.72)
    ax.set_ylabel("命中最优实例数（/40）")
    ax.set_title("ma-QAOA 构造性支配（6×6 子空间变分区，实测）", fontsize=15, fontweight="bold", pad=12)
    ax.legend(frameon=False, loc="upper left")
    cap_ax(ax, -0.145, "支配是定理：multi 以 layer 最优角展开态为种子（逐位相同）＋只接受严格改进 $\\Rightarrow\\ \\langle E\\rangle_{multi} \\leq \\langle E\\rangle_{layer}$；\n全空间坍缩读数已饱和、welfare 增益边际——如实不宣称。")
...
def fig_market():
    ...
    fig.suptitle("市场机制研究线：智力资本市场 + 在线校准（真实 LLM 实验，n=2112）", fontsize=15, fontweight="bold")

    a = a1
    a.set_xlim(0, 12); a.set_ylim(0, 10); clean(a)
    a.set_title("增广 WDP + Clarke 支付闭环", fontsize=11.5)
    box(a, 0.3, 7.6, 3.2, 1.7, "任务批 + 报价 bids\n（DSIC 域）", fc=TINT[GRAY], ec=GRAY, fs=9.0)
    box(a, 8.55, 7.6, 3.15, 1.7, "联合分配 X*", fc=TINT[GREEN], ec=GREEN, fs=9.3)
    box(a, 3.75, 7.6, 4.55, 1.7, "增广 WDP\nargmax Σ(v·$\\hat q$ − b) + Σg", fc=TINT[INDIGO], ec=INDIGO, fs=9.0)
    box(a, 4.15, 5.0, 3.6, 1.6, "Clarke pivot 支付\nDSIC 定理成立", fc=TINT[CYAN], ec=CYAN, fs=9.3)
    box(a, 0.3, 2.2, 5.3, 2.0, "在线校准：结算流 →\nBernoulli 极大似然 $(\\hat\\alpha,\\hat\\beta,R^2)$", fc=TINT[AMBER], ec=AMBER, fs=9.3)
    box(a, 6.6, 2.2, 5.1, 2.0, "$\\hat q(k)$ 学习曲线 + g 增长影子价值\n级数和天然有界", fc=TINT[AMBER], ec=AMBER, fs=9.3)
    ...
    b.text(3, 0.775, "实测 0.458→0.747（+0.289）", fontsize=8.8, color=INDIGO,
           bbox=dict(fc="white", alpha=0.85, ec="none", pad=1.2))
    ...
    b.text(44, 0.285, "污染 = 负资本\n（增益 −0.704）", fontsize=8.8, color=RED, ha="center", linespacing=1.5,
           bbox=dict(fc="white", alpha=0.85, ec="none", pad=1.2))
    b.set_xlabel("累积案例数 k")
    b.set_ylabel("隐性技能 $\\hat q$")
    b.set_title("学习曲线与污染坍缩（30 例后污染，示意重建）", fontsize=11.5)
    b.set_ylim(0, 0.9)
...
def fig_execution_tier():
    ...
    ax.set_xlim(0, 12); ax.set_ylim(1.35, 8.9)
    clean(ax)
    ax.set_title("v1.11 执行层级路由：提交前先过 FTQC 资源估算器", fontsize=15, fontweight="bold", pad=12)

    box(ax, 0.3, 6.7, 3.3, 1.6, "新任务提交\n（电路画像：比特数 / 深度 / T 数）", fc=TINT[GRAY], ec=GRAY, fs=9.3)
    box(ax, 4.2, 6.7, 3.6, 1.6, "FTQC 资源估算器\nqLDPC gross [[144,12,12]] / surface\n码目录 · 显式假设表", fc=TINT[INDIGO], ec=INDIGO, fs=9.2)
    arrow(ax, 3.7, 7.5, 4.15, 7.5, color=GRAY)

    box(ax, 8.6, 7.2, 3.1, 1.2, "① NISQ 直发\n资源在容错预算内", fc=TINT[GREEN], ec=GREEN, fs=9.3)
    box(ax, 8.6, 5.4, 3.1, 1.4, "② FTQC 等待队列\nFtqcDeferredError\n携带完整资源画像", fc=TINT[AMBER], ec=AMBER, fs=9.3)
    ...
    ax.text(6, 2.35, "边界如实：估算是资源画像不是硬件声明；三态路由本身由 execution-tier 测试钉住（14 用例：数值内核 · 三态 · 接缝）",
...
def fig_quality():
    import numpy as np
    fig, (a, b) = plt.subplots(1, 2, figsize=(12.6, 4.9), gridspec_kw={"width_ratios": [1.28, 1]})
    fig.suptitle("质量门禁体系：六道门禁 + 覆盖率棘轮（2026-09 全量绿）", fontsize=15, fontweight="bold")

    a.set_xlim(0, 11.4); a.set_ylim(0, 10); clean(a)
    gates = [
    ...
    for i, (name, desc, color) in enumerate(gates):
        y = 8.6 - i * 1.42
        lines = desc.split("\n")
        a.add_patch(FancyBboxPatch((0.2, y - 0.5), 4.45, 1.15, boxstyle="round,pad=0.02,rounding_size=0.05", fc="white", ec=color, lw=1.5))
        a.text(2.42, y + 0.08, name, ha="center", fontsize=9.6, fontweight="bold", color=DEEP)
        a.text(2.42, y - 0.27, lines[0], ha="center", fontsize=7.6, color=GRAY)
        a.add_patch(Circle((5.15, y + 0.08), 0.16, fc=color, ec="none"))
        a.text(5.5, y + 0.08, lines[1] if len(lines) > 1 else lines[0], ha="left", va="center", fontsize=8.6, color=GRAY)
    ...
    for i, (c, t) in enumerate(zip(cov, thr)):
        ...
        b.text(i, t + 1.0, f"门槛 {t}%", ha="center", va="bottom", fontsize=7.4, color=RED,
               bbox=dict(fc="white", alpha=0.85, ec="none", pad=0.9))
    b.plot([], [], color=RED, lw=2.2, label="防回归门槛")
    b.set_xticks(x, names)
    b.set_ylim(75, 100)
    b.set_ylabel("覆盖率（%）")
    b.set_title("c8 覆盖率 vs 防回归门槛", fontsize=11.5)
    b.legend(frameon=False, loc="lower right", fontsize=9)
...
def fig_hotpath():
    ...
    ax.set_title("平台热路径性能（经典 hybrid 模式，npm run performance 一键复现）", fontsize=15, fontweight="bold", pad=12)
    for b, v, m in zip(bars, vals, mult):
        ax.text(b.get_x() + b.get_width() / 2, v * 1.25, f"{v:,}\n优化 {m}", ha="center", fontsize=9.3, color=DEEP, fontweight="bold", linespacing=1.4)
    cap_ax(ax, -0.185, "实测环境 Node.js v24 / Windows / 2026-08；绝对吞吐随机器与热状态浮动，相对优化倍数是稳定口径。\n>10³ tasks/s 的高吞吐热路径仍走经典启发式（诚实边界）。")
...
def fig_timeline():
    ...
    arrow(ax, 0.4, 5, 12.25, 5, color=GRID, lw=3, ms=22, zorder=1)
    marks = [
        (1.15, "v1.0", "平台基座\n经典启发式调度\n+83× 性能优化", 1),
        (2.55, "v1.1", "真实量子物理\n复振幅态矢量 · QAOA\n绝热退火 · Born 坍缩", -1),
        (3.95, "v1.2", "约束子空间\n纤维闭式混合器\n等效 80 量子比特", 1),
    ...
    CW, CH = 2.15, 2.30          # 卡片宽高（同侧相邻卡片间距 ≥2.2，宽 2.15 留 0.05 缝）
    for x, v, desc, side in marks:
        ax.add_patch(Circle((x, 5), 0.14, fc=INDIGO, ec="white", lw=1.5, zorder=4))
        if side > 0:
            cy = 6.55                       # 卡片底部
            ax.plot([x, x], [5.12, cy], color=INDIGO, lw=1.2, zorder=2)
        else:
            cy = 5 - 0.35 - CH              # 卡片底部（镜像）
            ax.plot([x, x], [cy + CH, 4.88], color=INDIGO, lw=1.2, zorder=2)
        card = FancyBboxPatch((x - CW / 2, cy), CW, CH, boxstyle="round,pad=0.02,rounding_size=0.08",
                              fc="white", ec=INDIGO, lw=1.3, zorder=3)
        ax.add_patch(card)
        BOXES.append((ax.figure, card, f"card {v}"))
        ax.text(x, cy + CH - 0.42, v, ha="center", va="center", fontsize=10.5, fontweight="bold", color=INDIGO, zorder=4)
        ax.text(x, cy + 0.24, desc, ha="center", va="bottom", fontsize=7.9, color=DEEP, linespacing=1.55, zorder=4)
...
if __name__ == "__main__":
    ...
    if COLLIDES:
        print(f"!! COLLISION WARNINGS: {len(COLLIDES)} text/box collision or out-of-axes case(s):")
        for c in COLLIDES:
            print("   ", c)
    else:
        print("collision self-check: 0 text/box collisions, 0 out-of-axes escapes")
    sys.exit(1 if (glyphs or OVERFLOWS or COLLIDES) else 0)
