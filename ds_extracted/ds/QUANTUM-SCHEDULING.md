...
> （v1.11 勘误：0/5 为示例脚本半账记账 Artifact，统一记账下经典侧贪心/
> 局部搜索各 2/5——见第六节勘误注。）
>
...
- 耦合赛道（6×8，纠缠对使问题变为 QAP 型 NP-hard）：
  ...
  |---|---|---|
  | 贪心 | 2/5 | 4.502–5.158 |
  | 局部搜索 | 2/5 | 4.611–5.158 |
  | **量子子空间** | **5/5** | **4.858–5.158（=最优）** |

  > ⚠️ **勘误（v1.11，由 QuantumSched-Bench 定罪）**：本表早期版本载
  > "贪心 0/5、局部搜索 0/5"——那是示例脚本给经典侧只记**不含耦合加成的
...
## 九、测试覆盖（429 个量子/基线/QPU 用例 / 全套 1297 用例）
...
execution-tier 14 + twin-convergence 13（v1.12 单源化对拍）+ qpu-r10 9（R10：QPU 注册零副作用 · 自适应轮询 · 幻影解码拒绝 · 零福利最优）= 124，
后续波次增量 205（R13–R17 各波量子/QPU 域用例 124→329）＋R18 增量 51
（r18b 噪声感知选择器 14 + 对易 FT 并行画像 14 + r18d γ 谱买断 23）＝ 380，
R19 增量 49（r19q γβ 交替精修 9 + 非均匀采样买断 18 + r19s 执行层级双画像 13 +
Misra-Gries 分解 9）＝ 429。
...
npm test                # 1297 用例 · 0 失败（含 429 个量子/基线/QPU 用例）
