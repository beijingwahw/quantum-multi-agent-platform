...
| [质量门禁](#-质量门禁体系) | 1033 用例 · 六道门禁 · 覆盖率棘轮 |
| [版本演进](#️-版本演进时间线) | v1.0 → v1.16 |
...
**1033 用例 · 0 失败 · 91 个测试文件 / 309 个套件**；六道门禁全绿，覆盖率棘轮只升不降：
...
**v1.0** 平台基座（经典启发式 + 83× 性能优化）→ **v1.1** 真实量子物理 → **v1.2** 约束子空间 → **v1.3** 认证基线 → **v1.4** 真 QPU 后端 → **v1.6** 确定性并行（23.5×，逐位一致）→ **v1.9/10** CVaR + ma-QAOA（定理级支配）→ **v1.11** QuantumSched-Bench + FTQC 分级 → **v1.12** 质量交付波（错误面收敛 · 单源化孪生收敛 · 本图集）→ **v1.13** 创新波（11 个 opt-in 新模块，749→937 用例）→ **v1.14** 缺陷清偿波（七项登记缺陷红测修复＋文献双源核实 44 条，937→961 用例）→ **v1.15** ECCM（两值谱单余弦定理：三点定弦·闭式全局极小·零步长超参，基准族 11/12 实例对坐标下降双轴同时占优，961→982 用例）→ **v1.16** 严格正向升格（四个条件性创新的代价面逐一消除：位势流 SPFA 平价·准入精确拒绝·批组成零成本路径·Hedge 组合后悔界，982→1033 用例）。
