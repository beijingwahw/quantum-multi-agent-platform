...
export function assertStinespring(st: Stinespring, tol = 1e-12): void {
  if (!matEq(mMul(mDagger(st.V), st.V), identity(st.d), tol)) {
    refuse('STINESPRING_ISOMETRY', 'Stinespring dilation failed the isometry check V†V = I');
  }
}
...
export function branchIsometry(va: Stinespring, vb: Stinespring, first: 'A' | 'B'): CMat {
  if (va.d !== vb.d) refuse('BRANCH_DIM_MATCH', 'target dimensions must match');
...
export function switchIsometry(va: Stinespring, vb: Stinespring): SwitchIsometry {
  if (va.d !== vb.d) refuse('SWITCH_DIM_MATCH', 'target dimensions must match');
  const d = va.d;
  const eA = va.envDim;
  const eB = vb.envDim;
  ...
  if (!matEq(mMul(mDagger(M), M), identity(2 * d), 1e-12)) {
    refuse('SWITCH_ISOMETRY', 'switch isometry failed M†M = I — construction bug');
  }
