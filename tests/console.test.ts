...
describe('Web控制台协议 端到端', () => {
  ...
  it('控制台完整闭环：查询→提交任务→添加Agent→完成任务', async () => {
    ...
    await waitFor(latest, (snap) => snap.agents.some((a: any) => a.name === 'Console E2E Agent'));
    ...
    const done = await waitFor(latest, (snap) =>
      snap.tasks.some((t: any) => t.id === task.id && t.status === 'completed'),
    );
    // 终态快照与指标双确认（而非仅"未抛错"）
    const completedTask = done.tasks.find((t: any) => t.id === task.id);
    assert.equal(completedTask.status, 'completed');
    assert.ok(done.metrics.scheduler.completedTasks >= 1);
  });
