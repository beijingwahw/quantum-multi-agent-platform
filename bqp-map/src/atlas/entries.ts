...
export const ATLAS: readonly AtlasEntry[] = [
  ...
  },
  {
    ...
    certs: [
      { kind: "cross-prototype", ref: "qram-sched", note: "adversarial-stream decision buffers byte-identical, classical vs quantum argmax; v0.2.0 the KVV tight instances EXECUTED — three independent exact kernels agree to 1e-15 and reproduce Feige's a(n) table 1,3,13,67,411,2921,23633 with the additive constant 1−2/e; greedy exactly n/2 under both tie-breaks (n=8..128); the quantum read speedup 1.22×→6.20× on the tight stage with the ratio caps unmoved; the barbell k=2..4 negative holds; qRAM metered — insertion amortization breaks even at T≥1493 tasks at n_b=20. The 'KVV tight instance not reproduced' boundary is retired" },
    ],
  },
  {
    id: "online-matching",
    ...
    certs: [{ kind: "cross-prototype", ref: "qram-sched", note: "1−1/e and 1/2 caps machine-verified; v0.2.0 the KVV tight family itself EXECUTED (D_n/MonotoneG per Feige arXiv:1812.11774): three independent exact kernels agree to 1e-15, Feige's table and the 1−2/e additive constant reproduce, greedy is exactly n/2 under both tie-breaks, and the quantum read speedup 1.22×→6.20× on the tight stage leaves the ratio caps unmoved — the caps now stand on the adversarial family, the 'not reproduced' boundary is retired" }],
  },
  {
    id: "stoq-anneal",
  ...
  },
  {
    ...
    certs: [
      ...
      { kind: "cross-prototype", ref: "ds_extracted/ds", note: "platform benchmarks: NP-hard track 5/5 vs best classical 3/5 (v1.11 unified-accounting erratum — the earlier 0/5 was a half-ledger artifact; npm run bench reproduces)" },
    ],
  },
  {
  ...
  },
  {
    ...
    certs: [
      ...
      { kind: "cross-prototype", ref: "postselect-sched", note: "restart-scheduling theorem layer: the ledger placed inside executed LSZ93 restart theory (no-beating by exhaustive enumeration, convex-combination identity, universal-doubling robustness, k=0-optimal threshold law at density (3-sqrt(2))/4, heralded-loss freeness accounting); v0.2.0 the tie face: the typed-refusal protocol at exact ties (half-binomial sum 2^(k-1) exact), 58 tie tables at d <= 64 verified both directions, the tied-menu plateau 14/14, and the c_k census at 40 certified digits with c_1 > c_2 > ... > c_6" },
      { kind: "cross-prototype", ref: "survivor-census", note: "survivor semantics on the WEIGHTED prior: the kept branch is the posterior over optima (BAY63's inverse step on branch amplitudes, integer-ratio referee; phases survive — coherent survivor, dephased posterior reading); the kill register itemized per universe, summing to 1-P exactly; waiting price E[T]=1/P with the exact (1-P)^k schedule vs the conservative bound; boundaries executable (P=0 refusal, unfunded optimum never returns, P=1 empty register); v0.2.0 S6 composition laws exact — chain rule P_AB = P_A*P_2 to 1.11e-16, staged kill registers disjoint with union deviation 0, the renewal E[T_AB] identity to 8.88e-16, amortized odds ADD; S7 phase census — ledger deviation exactly 0 across 9 encoding families, Fourier visibility 0.117851 two-path" },
  ...
  },
  {
    ...
    certs: [
      ...
      { kind: "cross-prototype", ref: "retro-cache", note: "the tariff ledger: pre-arrival equivalence with a shared seed (TV = 0), the 256-strategy classical census capping CHSH at exactly 2 (CHSH69) vs the singlet's 2*sqrt(2) (TSIR80), and the priced withdrawal exchange rate (QBER two-path, reconciliation floor h2 per SHAN48, settings tariff); v0.2.0 BB84-grade bounded privacy amplification: the exact universal-2 census 15/255, Eve's information 1.510→0.182 at ε = ¼ with the gap to the 1−h₂ line equal to I_E exactly, the LHL vacuous at toy scale printed honestly, the full tap ABORTed; and the active-adversary census with settings mutation = strict tax" },
      { kind: "cross-prototype", ref: "nosignal-tariff", note: "the itemized schedule, one checker for the whole correspondence's correlators: 24-axis zero leakage under local unitary + CPTP (floor 2.3e-16), the order register's input-blind bit (1.7e-16), the HJW ensembles' unreadable commitment (pairwise TV = 0), and the withdrawal curve net(p) = (1-h2((1-p)/2))/2 executed as a full monotone schedule with exact anchors and h2 on two independent paths; v0.2.0 the T4 interior data→theorem: monotonicity's exact machine certificate (min gap 9.02e-4, widest enclosure 8.97e-22, Δ² bound 0.00181), the convexity/inflection face, and the SIC-POVM tetrahedral census floor unchanged at 1.11e-16" },
  ...
  },
  {
    ...
    certs: [
      ...
      { kind: "cross-prototype", ref: "route-price", note: "D1: the route-and-price dossier — criteria, falsified milestones, and the Landauer-floor price lines re-derived by executable witnesses (k exact, ln2 by quadrature); v0.2.0 the independent zero-dependency cross-checks: the echo identity rebuilt inline to 2.22e-16, the own 11-gate multiplier 16/16, orbit flatness 2.66e-15, W_0 0.256552 vs the sibling's 0.2566, and the tariff 0 < 5 < 9 < 43.02 re-priced on the own netlist (D1-M3/D1-M4/D2-M4 receipted CERTIFIED-ELSEWHERE)" },
      { kind: "cross-prototype", ref: "dtc-clock", note: "the open core executed at the model layer: beat-keyed universal reversible computation (cargo exact), the legislated tariff table (0 < 5 < 9 < 43.02 units), zero-net-work and detuned-payment laws, the frontier census, and the WO15 tombstone — every number re-derived by the sibling's own witness battery on every suite run (the board and witness counts are machine-audited THERE by npm test, never quoted here); v0.20.0 TWO CONVICTIONS of previously-shipped claims, both machine-convicted by the new tests: (i) the TC13(ii)/TC18 echo-decay law |cos2d|^k had been verified TAUTOLOGICALLY (the v0.2.0 witness certified the formula against itself) and is FALSE — the true isolated-echo law is m(k) = (-1)^k cos(2k*delta) exactly (8.3e-15), and the geometric law is re-founded by TC46 as exact-in-expectation under per-period sign noise (exhaustive over all 2^k sequences, 3.6e-15), with route-price credited as the discovering sibling; (ii) zetaEM ADDED a spurious (1/2)N^-s tail term — the v0.19.0 'Phi1 = -4.547e-4 small but NONZERO' was that bug's artifact: Phi1 now bracketed +7.14e-8 within |Phi1| <= 5.58e-7 CONTAINING ZERO (fate open), kappa = -0.306852819, and the sigma1 transfer confirmed 5.8e-6 on an independent incremental-binomial road" },
    ],
  },
  {
    ...
    certs: [
      { kind: "cross-prototype", ref: "quantum-mech", note: "T3: GHZ double-collateral ceiling 1/sqrt(2), threshold > 1/2 admits at most one party; v0.2.0 data-locking executed at bounded exact scale: the locked ensemble's χ bracket [0.131, 0.177] bits while the data grows to 6 bits at K = 4d, the key sweep 0.705→0.044, and the erasure-robustness census — OTP hiding χ ≡ 0 under any noise machine-verified law, HJW concealment survives dephasing exactly, intercept detection drowns at γ = 1 damping (honest)" },
      { kind: "cross-prototype", ref: "ent-clearing", note: "the SETTLEMENT layer executed as its own board: teleportation burns the coin to deliver (identity channel at machine precision, post-trade concurrence exactly 0, goods frozen until the 2-bit classical leg settles); dense coding returns it as a catalyst (exactly 2 cbits, post-decode concurrence exactly 1); Procrustean netting at exactly 2*l_min; the mint wall — local channels never raise E_F, one global CNOT mints C 0->1; v0.2.0 the purification desk: BBPSSW rounds exact to 5.6e-16 with the 24-element Clifford twirl machine-measured load-bearing (without it the nested round degrades 0.884->0.812); yield curves n=2,3,4 — F rises 0.884->0.913 while coin-yield falls 0.410->0.144 at F=0.85; the hashing line QUOTED with the sign change bracketed between F=0.81 and F=0.82; the GHZ mint wall refuted per-pairwise (one LOCC withdrawal moves C_AB 0->1), surviving per-cut with a 150-round local-channel census rise of 0" },
  ...
  },
  {
    ...
    certs: [
      { kind: "cross-prototype", ref: "quantum-mech", note: "T4: perfect hiding (T at 1e-16) while every reveal passes at exactly 1/2; v0.2.0 the HJW-under-noise census (concealment survives dephasing exactly, drowns at γ = 1 damping) referenced" },
      { kind: "cross-prototype", ref: "binding-price", note: "the market of this no-go executed: the one-coin identity (binding slack = half the marginal's polarization — concealment loss one-to-one, dev 1.3e-16) and the witnessed flat supply (the entire HJW strategy space leaves the reveal at exactly 1/2 — no decomposition moves it at any offer); the classical cheat's 0.853553 re-derived as the frontier point r = 1/sqrt(2); v0.2.0 G6 the continuous-family flat supply (2141 parameterized ensembles, worst TV 8.3e-17), G7 the identity-under-noise census (the bend w = 0.103553 at gamma = 1/2, the confiscation break gamma/2), G8 the two-coin census — per-coin flat, joint reveal movable 1/4->1/2, a found boundary" },
  ...
  },
  {
    ...
    quantumUpper: "the symmetry is exhibited and machine-verified at BOTH layers: the Groves GAUGE GROUP (payments modulo h_i(b_-i), GL79) with the WELFARE GAP Phi_v(x(b)) - max Phi_v as the conserved charge (bitwise gauge-invariant, <= 0, = 0 at truth); implementability = exactness of the payment 1-form dp = -d(W_-i . x) (Rochet cyclical monotonicity: no positive cycle); the same algebra as the discrete Noether theorem executed on a variational integrator (charge drift 2.4e-14 symmetric vs 2.0 broken — fourteen orders of separation); and (v0.2.0) the CONTINUUM derivation executed: on the quadratic divisible-good family every link of [E]nvelope -> [I]ntegration (Poincare/FTC, the gauge readoff) -> [S]tationarity (efficiency) -> [G]roves form p = h(theta_-i) - W_-i(x) is the ZERO POLYNOMIAL in exact rational arithmetic — it holds for every type profile in the region at once; the charge in closed form -(n-1)(s-t)^2/(2n) (DSIC by ordered-field arithmetic); Noether I = gauge-orbit conservation of the charge (dG/deps = 0 coefficient-wise); Noether II = the gauge identity whose moduli reading IS Green-Laffont uniqueness (the solver reads the gauge off any DSIC payment and convicts off-orbit payments)",
    quantumLower: "the executed derivation is the SMOOTH-CONVEX layer: smooth mechanisms on convex type regions with interior optima — four machine controls mark the load-bearing hypotheses (off-gauge crime priced eps^2 n/(2(n-1)) exactly; kappa-family implementable but non-Groves with drift exactly kappa(1-kappa)(n-1)/n s; the winding form closed yet 2*pi around the unit diamond — Poincare load-bearing, a strictly continuum shape no finite type set can host; the second-price kink locus). What stays excluded: the general measurable-space Green-Laffont (GL79, cited) and Noether's theorems as general statements (NOE18/KSS11, cited)",
    verdict: "MECHANISM-SETTLED",
    rationale:
      "The open question was: exhibit the symmetry group whose charge is truth-telling, or show none exists. Exhibited, both sides of one algebra, at BOTH layers: invariance under a group action => the contracted 1-form is closed (cycle sums vanish / closedness identity) => a potential (the charge) exists => stationarity (truthful reporting / DEL trajectory). v0.1.0 shipped the discrete isomorphism (integer-exact mechanism side, rounding-exact physics side, broken cases failing measurably). v0.2.0 executes the boundary the row still carried: Noether's 1918 continuum theorem -> Green-Laffont as a machine derivation — the chain assembles coefficient-exact in both directions, uniqueness is the Noether-II moduli reading, and each hypothesis of the theorem now carries a machine witness of its own load-bearing character. What remains open is only the epoch-5 superstructure — the choice-primitive row.",
    certs: [
      { kind: "cross-prototype", ref: "dsic-noether", note: "T1-T4 discrete layer: gauge orbit + welfare-gap charge bitwise; Rochet exactness; discrete Noether executed; the bridge table. T5-T8 continuum layer: the chain [E][I][S][G] as zero polynomials, the closed-form charge, Noether I orbit conservation, Noether II classification, four boundary controls (K1-K4). v0.3.0 the quartic BOTH ways: the externality-free stage closes exactly with charge -(s-t)^2(s^2+2st+3t^2)/4, and the coupled stage gets a no-polynomial certificate (factor identity + SOS + degree arithmetic); the K4 piecewise-smooth gluing exact ([u] = t[x] - [p], wall flux jump (t-w+1)/2)" },
      ...
      { kind: "citation", ref: "NOE18", note: "symmetries and conserved quantities — the continuum ancestor, now EXECUTED at its smooth mechanism instance" },
      { kind: "citation", ref: "GL79", note: "Groves schemes are the only DSIC mechanisms for efficient social choice — the gauge orbit IS the Groves class; the smooth-layer derivation is executed, the measurable-space theorem stays here" },
      ...
      { kind: "citation", ref: "KSS11", note: "the two-theorem structure — the gauge group C^infinity(Theta_-i) is a second-theorem class, its identity is the classification" },
      { kind: "citation", ref: "OLV86", note: "Helmholtz conditions / inverse problem — the allocation 1-form's closedness as the Euler-Lagrange face of the report action" },
      { kind: "citation", ref: "SPI65", note: "Green's theorem and the Poincare lemma — the [I] step's analytic content, executed as exact polynomial integration on a convex box" },
    ],
  },
  {
    ...
    certs: [
      ...
      { kind: "cross-prototype", ref: "causal-ineq", note: "the causal inequality executed: 8,192-strategy exhaustive 3/4 cap, W* violation exactly cos^2(pi/8), validity checker with derived pattern rules; v0.2.0: within ALL qubit-lab instruments the OCB functional F_q on W* has sup = cos^2(pi/8) EXACTLY (proven by functional relaxation, the OCB protocol attaining), W* ≡ OCB12 eq. (7) elementwise, and LC25's Tsirelson-type bound 1+1/sqrt(2) cited as the all-dimension anchor" },
      { kind: "cross-prototype", ref: "k-switch", note: "the 3-switch executed: parity-orthogonality readout fidelity 1 exact, structural plain-order blindness, six-order scheduling contact law D(fixed)/sqrt(2); v0.2.0 the k=4 face: the parity law <u|u_sgn> = 0 EXACT on all 24 orders (readout fidelities 1.0, 96-dim isometry M-dagger-M = I), the 256-census (136 Pauli sets readable with probability EXACTLY 1; fixed-order supersequence minimum 9 vs the switch's 4 uses), the honest NO-GO at d=4 (0 commuting Pauli quadruples in the 1365-census), and the halving law BREAKING at k=4: switch D = sqrt(10)/6 ~ 0.527, still below every fixed order's 1/sqrt(2)" },
      { kind: "cross-prototype", ref: "readout-wall", note: "the FIRST wall executed as its own object: the readout exchange ledger — ESC18 chi 0.048794940695399 lives entirely in the off-blocks and dies to exactly 0 under order readout, the bought order bit is input-blind (P=1/2), the dephased switch IS the fixed-order mixture (identity to 1e-15, second path), complementarity at the replacer pair, weak-readout curve as data, k=3 face included; v0.2.0 the E4 interior data->theorem: ESC18 chi and two more closed forms with dyadic-affine spectra certified STRICTLY decreasing and strictly convex by exact BigInt rational intervals, ln enclosed on two independent series (widths <= 1.6e-20, min gap 1.127e-4); the E6 complementarity frontier prices the first 1/20 of order knowledge at 0.00512 (ESC18) / 0.06511 (replacer) chi-bits; the k=3 chi closed form carries member spectrum {n/48}" },
  ...
  },
  {
    ...
    family: "genealogy",
    classical: { cls: "exact at benchmarked sizes", note: "Hungarian parity point-for-point on the linear track; strongest classical heuristics 3/5 on the NP-hard coupling track (v1.11 unified accounting — the earlier 0/5 was a half-ledger artifact the platform's own erratum corrected; volatile counts are machine-audited THERE by npm test, never quoted here)" },
    ...
    certs: [
      { kind: "cross-prototype", ref: "ds_extracted/ds", note: "the platform repo: Hungarian parity, NP-hard 5/5 vs best classical 3/5 benchmarks (unified accounting; suite counts are machine-audited THERE by npm test, never quoted here)" },
  ...
  },
  {
    ...
    verdict: "MECHANISM-SETTLED",
    rationale:
      "The open core was stability as PHYSICS, not compilation; that core is now executed. choice-lang had made the world stable by construction (engineered programs, charge conserved — the language layer); stable-world adds the physics layer: a fixed dissipative law whose absorbing class IS the marked world, reached from anywhere with no postselection and no arrival toll, held by a Lyapunov certificate on the same membership charge, robust under law-error, escape exactly impossible — and PAID FOR on both faces (the sector-erasure tariff on the settled #11 schedule, classical face; the coherent face priced at kT ln2 · C_rel per stabilization on the authored diagonal Hamiltonian — F(rho)-F(Delta rho) = kT ln2 · C_rel exactly, pure straddlers pay the full bit, erasure monotone at exactly twice the classical rate), READ in temperature (the thermal escape reading: Boltzmann logistic occupancy 1/(1+e^-beta-dE) exact, design rule beta-dE >= ln(K gamma/delta), 5 GHz at 10 mK reads 1-3.79e-11 in exact SI), and now DERIVED and BANKED (v0.3.0): the thermal rates are no longer authored — the microscopic collision model (resonant Gibbs qubits, energy-conserving exchange) derives detailed balance exactly and the law itself is the family's zero-temperature member (TD against applyLaw decaying as e^-beta-dE; the coherence factor cos theta is temperature-free); and the coherent shortcut (the same exchange at theta = pi/2 against a ground weight) reaches the law's k->infinity state in ONE step, conserves full-basis C_rel exactly, and banks the straddler's full sector bit on the weight with free-energy excess exactly kT ln2 — coherent protocols conserve what incoherent laws dissipate. The boundaries stay on the same line: the law is authored (machine layer; nature's instantiation not claimed, the vacuum-compiler precedent), the law selects the world never the contents (path-dependent), the shortcut needs a prepared coherent weight and harvesting beyond straddlers needs quantum side information (banked fraction 0.01-0.95 over the census), the continuum limit is EXECUTED as a discrete convergence theorem (finite coupling exact and closed-form, composition error O(t sin^2 theta), the Davies coherence rate 1/2 restored exactly), the generator is IDENTIFIED as the Lindblad operator with the Davies rates (O(theta^2) elementwise, uniform over states) — only the general-Hilbert-space operator-norm theorem stays cited (DAV74); the burn is audited (the three-term ledger exact, the inverse permutation restoring the input bit-exactly) and the QSI boundary is SPLIT — its one-shot incoherent core executed exactly (the phase-alignment l1 bank, up to 72.7% recovered), the genuine correlation coherence staying priced-and-cited (WY16), and the continuum Green-Laffont derivation stays excluded (#14/#16 unchanged). The row graduates OPEN -> MECHANISM-SETTLED because its stated open item — an executable model of stability as physics — now exists.",
    certs: [
      ...
      { kind: "cross-prototype", ref: "stable-world", note: "the physics layer: the desired world as the stable solution of a FIXED law — quiet on the world (deviation exactly 0), globally attractive (leakage (1-gamma)^k exact for every input, into-world collapse to 1.6e-13), the charge as Lyapunov function (gamma(1-V) exact; conserved under engineered programs — one functional, two regimes), eps-perturbation census under the exact bound, escape exactly 0, stabilization tariff on BOTH faces (classical h2 schedule + coherent kT ln2 · C_rel), thermal reading DERIVED microscopically (collision bath: the law = the T=0 member), coherent shortcut banked (one-step collapse, C_rel conserved, the straddler's bit on the weight at exactly kT ln2), continuum limit executed (discrete convergence theorem, Davies rate restored), audit ledger closed (three-term decomposition exact, inverse permutation bit-exact), generator identified (Lindblad with Davies rates, uniform), QSI core executed (phase-alignment l1 bank); v0.6.0 the bounded catalyst EXECUTED (the holder's Schmidt-purification memory rides the shortcut and harvests the correlation term rung by rung — alignment unlocks <= 0.238, the classical record <= 0.172, the bit ladder <= 0.469 bits, the cap log2 d_c TIGHT at d_c = 2, the gap census max 2.445e-21 J at 300 K), the two-world join law (qA + qB - qAB with qAB(k) = (1-gamma)^{2k} c0 to 7.41e-16, never a single geometric outside the trivial case), and the time-dependent error recursion priced exactly (the constant-eps limit meets AT4's bound to 8.33e-17)" },
      { kind: "cross-prototype", ref: "letter-audit", note: "the letter's own audit, graduated: v0.2.0 the frontier registry's OPEN face closed 2 -> 0 (#10 -> dtc-clock, #15 -> stable-world + choice-lang), every one of the 21 live pointers read from the siblings' reports on every suite run (the A6 live-pointer law — the same audit that caught its own needle quoting a sibling at the wrong precision)" },
    ],
  },
];
