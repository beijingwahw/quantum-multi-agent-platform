...
| Time crystals as the clock wall — zero-energy eternal beat | not applicable | model layer executed: an exact driven-echo family whose beat (F+ Z F = -Z exactly, pi-paired quasi-energies) advances a one-hot token once per subharmonic period, firing a universal reversible gate set with cargo fidelity 1 at every tick — and the tariff table legislated before the numbers: DTC-clocked Bennett machine 0 kT-ln2 units per run vs irreversible rival 9 (dtc-clock) | the equilibrium half is a no-go (WO15); the driven half's costs are priced, not waived — zero net work in the closed model, every use metered | citation:MI22; cross-prototype:route-price; cross-prototype:dtc-clock |
