...
export const ANCHOR_REGISTRY: readonly AnchorRegistration[] = [
  ...
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: version: \"0.20.0\"", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "genuine sibling citations pass the two-ground audit", evidence: "the version-pin guard (the version-pin cross-repo family's founding sighting, b80#7): the genuine-citation test reads the sibling's LIVE package.json at run time — a citation pinning a stale dtc-clock version (0.19.0 after the same wave's 0.20.0 upgrade) is convicted as 'version drift' on every suite run, and the fixed needles are pinned in the citing tree (test/docs/README/experiments); in a multi-repo same-wave upgrade every live-pinned version citation is re-verified after each batch" },
  ...
  { anchor: "mutant-census/package-lock.json :: 0.28.0", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the registrar's own lockfile-sync guard: both of the census's own package-lock.json version slots are pinned to the live 0.28.0 — the 0.9.0 residual stood nineteen version bumps behind in the very repo that enrolled the version-pin family, convicted at this wiring visit; a recurrence removes the needle and E3 convicts on every census run" },
