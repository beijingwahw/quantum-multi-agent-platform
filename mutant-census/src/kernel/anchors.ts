...
export const ANCHOR_REGISTRY: readonly AnchorRegistration[] = [
  ...
  { anchor: "letter-audit/src/kernel/frontier.ts :: tests-639", kind: "RESOLVED", evidence: "the needle is on disk in the citing tree and the frontier checker fires it against the sibling's README on every letter-audit suite run (registered the moment b89#10 first sat on it)" },
  ...
  { anchor: "burial-record/package-lock.json :: 0.8.0", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the registry's own lockfile-sync guard: both of burial-record's package-lock.json version slots are pinned to the live 0.8.0 — the 0.4.0 residual survived the 0.5.0 bump because the delivery verified package.json only, and visit eighty-two's 0.7.0 bump shipped the same drift again (0.6.0 slots, the b92#1 conviction); the needle follows every bump since (repointed 0.6.0->0.8.0 at batch 92, the old needle having sat ACCIDENTALLY GREEN through @types/node ^20.5.0's substring — a needle must quote its target's own digits, not a dependency's); a recurrence removes the needle and E3 convicts on every census run (the lock is grepped after every bump, no exceptions for the registry's own repo)" },
...
export function resolveAnchor(reg: AnchorRegistration): { ok: boolean; detail: string } {
  ...
  if (reg.anchor === "letter-audit/src/kernel/frontier.ts :: tests-639") {
    ...
    const cited = readFileSync(root, "utf8").includes('"tests-639"');
    const target = readFileSync(resolve(WORKSPACE_ROOT, "ds_extracted/ds", "README.md"), "utf8").includes("tests-639");
    return cited && target
