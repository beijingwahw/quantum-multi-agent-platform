...
export const ANCHOR_REGISTRY: readonly AnchorRegistration[] = [
  ...
  // ---- batch 87 (the quality wave's sixth and FINAL batch's conviction
  // sites — fifteen latent defects convicted at their own needles plus the
  // wave's one TRUE NUMERICAL case pinned by a seed-pinned regression anchor,
  // registered the moment the rows first sat on them) ----
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: gate/placement-out-of-range", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "the silent gate drop is dead", evidence: "the placement guard: an off-register embedTwoAdjacent is refused by name where v0.3.0 returned the SILENT IDENTITY — the gate vanished with the dimension still right (the pair-exceeds-register, fractional-index and out-of-range variants beside it); the trial fires on every suite run with the legal placements landing exactly where they always did" },
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: hang conviction: geometricAttempts rejects p outside", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "hang conviction: geometricAttempts rejects p outside (0,1] by name", evidence: "the domain guard: p=0 (the draw loop that could never fire — a caller-side infinite hang) and p=1.5 (the sub-unit mean) are both refused by name, p=1 held exact at one draw per trial — the trial completing at all is the regression proof, firing on every suite run" },
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: readout/clock-not-divisor", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "readout guards: a non-divisor clock and an out-of-range clock step are named", evidence: "the divisor guard: a non-divisor clockStates on a 28-dim state is refused by name where v0.3.0 truncated D and read past the state buffers silently — garbage with no signal; the out-of-range step and eigendecomposition-mismatch faces die at the same block on every suite run" },
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: amplify/epsilon-out-of-domain", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "epsilon smuggling: non-probability eps is rejected by name at the census AND the auditor", evidence: "the probability guard: eps = 3/2 is refused at BOTH doors where v0.3.0 produced a meaningless census (mc = 1, sigma = NaN) that the auditor then faithfully re-verified against the meaningless ground — the lie never gets built now, the auditor never sees it; the trial fires on every suite run" },
  { anchor: "vacuum-compiler/test/vacuum.test.ts :: hamiltonian/step-dim-mismatch", kind: "FIRING-INJECT", demoFile: "vacuum-compiler/test/vacuum.test.ts", demoName: "NaN-grid conviction: malformed grids and dim mismatches are rejected by name", evidence: "the step-matrix guard: a wrong-dimension step matrix is refused at buildPropagation and buildDressing's doors where v0.3.0 poisoned the propagation grid with NaN downstream into H_prop (the causal-ineq NaN-grid sibling); legal operands stay bit-identical on every suite run" },
  { anchor: "letter-audit/test/letter.test.ts :: a short entry table is named and rejected", kind: "FIRING-INJECT", demoFile: "letter-audit/test/letter.test.ts", demoName: "EA:MACHINE: a short entry table is named and rejected", evidence: "the machine-shape guard: a short entry table is refused by name where v0.3.0 read undefined as a digit into NaN arithmetic and answered a fake {halted:false}; the out-of-range digit that silently corroded the ones count dies at the neighboring trial — both firing on every suite run" },
  { anchor: "letter-audit/test/letter.test.ts :: EA:TOWER-SHAPE", kind: "FIRING-INJECT", demoFile: "letter-audit/test/letter.test.ts", demoName: "EA:TOWER-SHAPE: a non-positive literal height is rejected at construction", evidence: "the tower-domain guard: a non-positive literal height is refused at construction where v0.3.0 let it reach compareTowers and be ordered by the monotone-bound branch (the unfounded 2↑↑0 > x); EA:TOWER-DOMAIN refuses the hand-built smuggle at the comparison itself — defense in depth on every suite run" },
  { anchor: "letter-audit/package-lock.json :: 0.3.0", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the lockfile-sync guard: both of letter-audit's package-lock.json version slots are pinned to the live 0.3.0 — the 0.1.0 residual survived the whole 0.2.0 version because the wave's gates verified the manifest only (the b85#32-34 lockfile family's next own-repo face); a recurrence removes the needle and E3 convicts on every census run" },
  { anchor: "switch-sched/src/experiments/exp3-sched-contact.ts :: system-register median drifted — rng stream perturbed?", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the seed-pinned regression anchor (the wave's one TRUE NUMERICAL conviction): Table 3's sample is seed-pinned — the four statistics asserted exactly (median −0.149905, joint median +0.147796, 0/40 system positives, 34/40 joint positives) — so any future perturbation of the rng stream (the extra draw the double-draw bug burned, welding the seed stream to the defect and producing the retired −0.160158/36/40 numbers) moves them and dies at the assertion instead of silently re-rolling the reported statistics; the anchor sits in the repro's own body, firing on every render" },
  { anchor: "switch-sched/test/hardening.test.ts :: input state must be 2x2", kind: "FIRING-INJECT", demoFile: "switch-sched/test/hardening.test.ts", demoName: "definite-order process guard names a malformed input state", evidence: "the input-shape guard: firstPartyProcess refuses a non-2×2 input state by name where v0.3.0 indexed past its rows and produced a NaN process silently — the rejection trial fires on every suite run" },
  { anchor: "switch-sched/src/switch/chanlib.ts :: assertStinespring", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the certificate-truth guard: assertStinespring now EXECUTES before the dilation leaves the module where v0.3.0's comment claimed the isometry certificate was checked while the call stood unreached — a recurrence that removes the call removes the needle and E3 convicts the claim-code drift on every census run (the b76#17 fake-claims face, needle-held at the call site itself)" },
  { anchor: "switch-sched/package-lock.json :: 0.6.0", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the lockfile-sync guard: both of switch-sched's package-lock.json version slots are pinned to the live 0.6.0 — the 0.1.0 residual survived the whole 0.2.0 version (the lockfile family's wave-6 face; the needle followed the 0.3.0->0.4.0 bump at the R12 wave's sweep, the 0.4.0->0.5.0 bump at the R15 exp4 prose repair, and the 0.5.0->0.6.0 bump at the R21 GYNI plateau + dimension-invariance delivery); a recurrence removes the needle and E3 convicts on every census run" },
  { anchor: "wukong-crossval/test/xval.test.ts :: XVAL_SHELL_RANGE", kind: "FIRING-INJECT", demoFile: "wukong-crossval/test/xval.test.ts", demoName: "the shell kernels reject shell indices beyond the mass vector", evidence: "the shell-range guard: exactShellMass and depolShellMass refuse shell indices past the mass vector by name where v0.3.0 read undefined into silent NaN (the dimension-slot family's next hole); the trial fires on every suite run" },
  { anchor: "wukong-crossval/package-lock.json :: 0.4.0", kind: "FIRING-INJECT", demoFile: "mutant-census/test/enrollment.test.ts", demoName: "smuggle E3", evidence: "the lockfile-sync guard: both of wukong-crossval's package-lock.json version slots are pinned to the live 0.4.0 (re-pinned from 0.3.0 in the R21 closeout when the Chernoff discriminability lower bound + the root census landed) — the 0.1.0 residual survived the whole 0.2.0 version (the lockfile family's wave-6 face, third repo of the batch); a recurrence removes the needle and E3 convicts on every census run" },
  { anchor: "wukong-crossval/test/xval.test.ts :: applyRX's sign convention is pinned by hand", kind: "FIRING-INJECT", demoFile: "wukong-crossval/test/xval.test.ts", demoName: "applyRX's sign convention is pinned by hand: e^{+i (theta/2) X}, the conjugate of textbook RX", evidence: "the convention-disclosure guard (the doc-code mismatch convicted WITHOUT a sign flip, the freeze discipline honored): the hand anchor pins amp(|1>) = +i at θ = π — the implementation's true convention e^{+i(θ/2)X} = RX(−θ) where the comment claimed textbook RX — with theory.md's mixer-sign section and the X4 export contract naming the conjugated beta axis for the hardware day; a doc revert or a silent sign flip both die at the hand value on every suite run" },
