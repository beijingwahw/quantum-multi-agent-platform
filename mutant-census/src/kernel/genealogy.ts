...
export const FAMILY_RESOLUTIONS: readonly FamilyResolution[] = [
  ...
  { family: "cat:statistics", holds: "GATE-ENFORCED", note: "the statistics class — the tier follows the LATEST sighting: b75#15's census completion that broke the tie equation's denominator is gate-held (exact tie iff d | a1^2, both directions, every reported tie re-verified through the full ledger row machinery), as are b75#14's halved U_1 initialization (the integer-ratio referee) and b75#2's 4x coefficient (the exact-eigenvalue anchor); b74#15's fabricated census size (20480 against the true 8192) and b74#14's global-½ double-count died at their anchors the same way (the fake-number family, closed at both faces); the arrangement members stand booked in their own rows (b73#4's beyond-noise-floor fit, b55#6's cancellation face), as do the census-level kills in earlier sightings' own rows (the MU8/MU9 lineage)" },
  { family: "cat:toolchain", holds: "GATE-ENFORCED", note: "the build/toolchain class — the tier follows the LATEST sighting: b75#18's latent repro no-op (v0.1.0's run-all imported the exp modules and the entry guard suppressed their mains — exit 0 fake green) is gate-held at the run-all needle (the explicit main calls ARE the fix; removing them removes the needle), as were b75#16's template-string quote mismatch (typecheck) and b75#17/b75#1/b75#5's lint batches; the shell-discipline members stand booked in their own rows (b74#0/b74#2's exit-code masking, b74#9's pre-machine require(), b74#11's apostrophe, b75#13's heredoc act), and the runner-path residue lives in the runner-path family's own row" },
  { family: "cat:conjugation", holds: "GATE-ENFORCED", note: "the convention class — the tier follows the LATEST sighting: b75#10's Pauli-ray phase defect (rotationsOf landing off the generator's ray) is gate-held at the matched-pair contract's phase assertion (product phase-only off the generator, diagonal exactly 0); b74#13's latent CJ-convention defect (identity not mapping to SWAP under complex Kraus, alive since v0.1.0) and b66#4's sequential-substitution slip were the previous; the physics face (transpose/dagger) stays MUTANT-KILLED in its own lineage (MU1)" },
  ...
  { family: "cat:citation-drift", holds: "GATE-ENFORCED", note: "the citation class — the tier follows the LATEST sighting: b75#7's latent VDL23 article-number defect (5807 with no DOI, shipped in this wave's first v0.2.0 delivery and caught by the sibling causal-ineq agent's double-source check) is gate-held at the corrected citations needle — the DOI is pinned on disk and the enrollment holds it there (the identifier family's first needle-held sighting); the booked members stand in their own rows (b74#5's deleted 'decision transformer', b56#9's Davies year, b55#3's arXiv digits — the double-source ritual stays author discipline for claims no needle can pin), as do the earlier machine-held members" },
  { family: "shell-template-heredoc", holds: "BOOKED-UNENFORCEABLE", note: "the banned heredoc/template family, twelve canonical sightings since b10 — the twelfth (b75#13, one scratch move in the k-switch delivery, not repeated) was grepped against the family's in-registry wrong-text sightings (16) before enrolling; the rule lives in the daily memory; Write tool always, no content-based exceptions" },
