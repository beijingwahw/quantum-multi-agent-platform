...
export const FAMILY_RESOLUTIONS: readonly FamilyResolution[] = [
  { family: "cat:wrong-object", holds: "GATE-ENFORCED", note: "the physics-object class — the tier follows the LATEST sighting: b76#22's inverted inflection map (nosignal-tariff's certificate named the certified-healthy cells as the suspects) is gate-held at the no-inflection-cell-named anchor with the fake-inflection-table smuggling trial beside it; b76#2's degenerate on-grid point setting the QAE requirement (p = 1/2 is the phase grid's best case, not the worst bank) and b76#5's averageRhoDefect comparing against I instead of I/d died at their population/object anchors; b75#12's projection-vs-discriminator conflation (k-switch's distinguishability account treated the projector's action as the game's separation) is gate-held — the exact game matrix is asserted cell by cell (all 144 entries 0) with a counterfeit-separating-order trial beside it; b74#18's over-strict enforcer and b74#16's multi-edge overestimate died at their referees the same way; the booked members stand in their own rows (b73#3's mismatched-scale frame, b72#0's false invariant, b57's domain/superset slips, b56's check-object slips, b66#8's readonly-Map mutation rides its own lint-held row)" },
  { family: "cat:process", holds: "BOOKED-UNENFORCEABLE", note: "the author-discipline class — the tier follows the LATEST sighting: b76#23's two misleading display truncations (nosignal-tariff's printed digits hid the load-bearing differences; rewritten) is booked on the display face — print-layer honesty is the b29#4/b30#1 line; the machine-held members ride their own rows (b76#13's placeholder gf2Rank held at the rank-vs-brute-force anchor, b76#17's fake test claims held by the rewritten suite, b75#4's tampered 0.71 assertion convicted by the exact eta* = 1/√2 noise-threshold test, b74#7's fragile assertions convicted by the suite's own re-run, b73#0, b73#6, b73#9, b54#0, b54#2, b60#7, b61#1, b63#2, b67#1, b67#5, b67#6 on E7, b68#0, b68#1, b69#2, b70#1), as do the booked members (b76#3's dead scaffolding, b76#7's attempted prose-before-run, b76#8's docs-section destruction, b73#10's visit/batch resync, b73#1 scratch mechanics, b73#2 the O(K^2) hang, b71#0's detached duplicate gate launch, b70#3's memory prefix duplication, b70#2's never-fail assertion, b69#0's string residue, b69#1's memory anchor, b68#2's heading swallow, b65#1's cross-session stale read-state, b75#6's /tmp log placement on the outside-tree face)" },
  { family: "cat:machine-overruled", holds: "GATE-ENFORCED", note: "the overruled-expectation class — the tier follows the LATEST sighting: b75#9's matched blind-pair generator was rejected at T = 0.918 by the contract now asserted in-tree (max T < 1e-7 over all 24 plain orders), as was b75#8's assumed swap Pauli quadruple (the census holds ZERO commuting at d=4, 0/30/1335); every conviction pinned by a test that asserts the numbers' verdict (e.g. TC27's passive-beats-decoder)" },
  { family: "cat:dimension-slot", holds: "GATE-ENFORCED", note: "the dimension-accounting class — the tier follows the LATEST sighting: b75#11's 8-dim split-layout NaN (control 4 ⊗ target 2, re at i / im at 8+i) is gate-held at Algorithm 1's exact-readout assertion (probability exactly 1 on every promising set), and b75#3's transcribed slot order dies at the elementwise W* ≡ OCB12 eq. (7) anchor; the class-level shape-blind face stays MUTANT-KILLED in its own lineage (the mAdd MU3 mutants)" },
  { family: "cat:statistics", holds: "GATE-ENFORCED", note: "the statistics class — the tier follows the LATEST sighting: b76#21's wide-rational NaN (fToNumber's naive Number(n)/Number(d) on limbs past 2^53, caught on the rendered report's face) is gate-held at the decimal-long-division fallback needle, as are b76#20's 2x-too-small LN2 tail enclosure (the cross-route overlap check caught it on the spot; the tight-enclosure test holds it) and b76#14/15/16's retro-cache algebra/sign/mixing-rate defects (crossing-exact, the honest-ledger zero-tap anchor, and the strictly-positive-tax mutation census); b76#1's BigInt→Number overflow NaN at n≥256 is held at the scaled-division needle; b75#15's census completion that broke the tie equation's denominator, b75#14's halved U_1 initialization and b75#2's 4x coefficient were the previous; b74#15's fabricated census size (20480 against the true 8192) and b74#14's global-½ double-count died at their anchors the same way (the fake-number family, closed at both faces — b76#7's attempted prose-before-run sighting stays booked in its own row); the arrangement members stand booked in their own rows (b73#4's beyond-noise-floor fit, b55#6's cancellation face), as do the census-level kills in earlier sightings' own rows (the MU8/MU9 lineage)" },
  { family: "cat:toolchain", holds: "GATE-ENFORCED", note: "the build/toolchain class — the tier follows the LATEST sighting: b76#25's latent switch-sched repro no-op (run-all imported the exp modules, the entry guards suppressed the mains — exit 0 without a single report re-rendered) is gate-held at its run-all needle; the repro-no-op family WENT SYSTEMIC this wave — postselect-sched (b75#18), qram-sched (b76#4, the destructive subspecies: the wipe ran and the render did not), retro-cache (b76#19), switch-sched (b76#25) all needle-held, quantum-mech fixed in the same wave — and a batch-7 full-workspace 29-repo audit is priced; the wave's other gate-held toolchain member (b76#10's lint/typecheck batch) rides its own row, as did b75#16's template-string quote mismatch (typecheck) and b75#17/b75#1/b75#5's lint batches; the shell-discipline members stand booked in their own rows (b74#0/b74#2's exit-code masking, b74#9 and b76#0's pre-machine require(), b74#11's apostrophe, b75#13/b76#9/b76#24's heredoc acts), and the runner-path residue lives in the runner-path family's own row" },
  { family: "cat:conjugation", holds: "GATE-ENFORCED", note: "the convention class — the tier follows the LATEST sighting: b76#6's traceProd sign bug (the im-im product entered with a minus where Tr[AB] for Hermitian a, b sums both products) is gate-held at the formula pinned in the kernel (Tr[AB] = Σ_ij a_ij conj(b_ij), real by construction); b75#10's Pauli-ray phase defect (rotationsOf landing off the generator's ray) is gate-held at the matched-pair contract's phase assertion (product phase-only off the generator, diagonal exactly 0); b74#13's latent CJ-convention defect (identity not mapping to SWAP under complex Kraus, alive since v0.1.0) and b66#4's sequential-substitution slip were the previous; the physics face (transpose/dagger) stays MUTANT-KILLED in its own lineage (MU1)" },
