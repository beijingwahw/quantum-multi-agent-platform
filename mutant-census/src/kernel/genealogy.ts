...
export const FAMILY_RESOLUTIONS: readonly FamilyResolution[] = [
  ...
  { family: "runner-path", holds: "BOOKED-UNENFORCEABLE", note: "the runner-path family (tsx -e Git Bash, PATH, entry paths, import homes, domain paths) — the tier follows the LATEST sighting: b84#19's banned-shape baseline probe (the wiring agent's own family byte-identity check run as node --import tsx -e with a relative dynamic import — the b14#4/b38#1/b46#1/b49#0/b83#8 line; it succeeded with output verified present, and success does not change the shape) is booked on the environment face — the act's failure mode is silence, the in-repo Write-tool probe or the suite's own W-D witness is the honest route; the previous was b83#8's silent tsx -e double failure (the nosignal-tariff delivery's RNG freeze-anchor probe, two consecutive silent runs with a relative import; the in-repo Write-tool probe plus run-and-delete is the fix, executed on the third attempt); the gate-held members stand in their own rows (b73#8's beyond-domain probe of the exact c3 path, held at the suite's n<=16 domain guard — G1 matched its 'path' text, the machine classed it), as do the repro-gate-held members and b60#3's import-grep residue" },
