...
export const FAMILY_RESOLUTIONS: readonly FamilyResolution[] = [
  ...
  { family: "cat:anchor-blindspot", holds: "GATE-ENFORCED", note: "the anchor-blindspot class — the tier follows the LATEST sighting: b84#0's transposed dagger2 subscript (locking.ts wrote -u.im[2] against the Y-basis conjugation's -u.im[1], breaking Y-basis rotations) is the family's HEAVIEST evidence: ALL FIFTY-SIX tests stayed green (the argmax readouts masked the leak) and only the repro byte-comparison killed it (exp7's honest gamma=0 line moving 0.0000 -> 0.0473) — gate-held at the Y-basis U-dagger-U both-sides anchor (H alone is real symmetric and immune; the anchor lives in the space that can actually fail — the class's rows stood at 17 in-registry, grepped before enrolling); the previous was b79#7's frontier needle quoting the CHSH constant at 15 decimals against retro-cache's printed 12, gate-held at A6 (letter-audit's own live-pointer law — every needle read from the sibling's report on every suite run, so a needle that misquotes its target dies the moment it lands); the intent-diffing residue stays booked in its own rows, and the named subfamily edit-anchor carries its own row (b53#5 itself files under runner-path: the machine's regex, not the author's reading of the lesson)" },
  ...
  { family: "count-drift", holds: "GATE-ENFORCED", note: "the count-drift family (b36 board literals, b37 dual lists, b45#9 prose, the b35 'four' found by B7's own pre-flight) — held since v0.5.0 by burial-record's B7 gate: stated counts must equal carried counts; the tier follows the LATEST sighting b85#11 (choice-lang's BRANCH_SHAPE guard conviction, filed here by the machine's first-match-wins on 'row count' in its text — gate-held at its own trial, the filing follows the rule, never the intent)" },
