...
export const REPAIR_AUDIT: readonly RepairRow[] = [
  ...
  // ---- batch 88 (the visit's FINAL wiring — born audited at burial) ----
  { key: "b88#0", verdict: "HELD", basis: "patch-act face (the b48#2/b53#0/b67#4/b82#6/b83#20/b86#6 line, ninth refusal): the Edit-before-Read refusal fired — the tool's own guard is the enforcement, but it is not a scheduled gate; nothing damaged landed" },
  { key: "b88#2", verdict: "HELD", basis: "act face: the redundant Read was refused mid-flight — the inverse of the Read-first line; which Reads were necessary is author-side sequencing no scheduled gate audits" },
  { key: "b88#3", verdict: "HELD", basis: "pre-machine face: the extraneous cast was deleted on self-review before any gate ran — its landing face is the same lint death route-price took one repo earlier; no gate compiles an editor buffer" },
  { key: "b88#6", verdict: "SHARPENED", basis: "dual-face (the exit-code-masking family's twenty-third attempt, its FIRST PIPESTATUS variant — the orchestrator's own self-report): the shell-act face — the suite piped through tail with the code echoed from PIPESTATUS — is booked; an echo is verdict-by-proxy, the shape banned regardless of what it displayed, and the no-pipe re-issue the only verdict. Booked: the shell-act face only" },
  { key: "b88#7", verdict: "SHARPENED", basis: "dual-face (the family's twenty-fourth attempt, second PIPESTATUS variant — the orchestrator's wave-7 red-cell handling): the shell-act face — the suite piped through grep under a PIPESTATUS echo — is booked; no verdict was taken from the pipe, the re-run read the red cell in full. Booked: the shell-act face only" },
  { key: "b88#8", verdict: "SHARPENED", basis: "dual-face (the family's twenty-fifth attempt, the third PIPESTATUS variant in two waves): the shell-act face — the repro piped through tail under a PIPESTATUS echo — is booked; display truncation is not a verdict, the re-run's direct code is. Booked: the shell-act face only" },
  { key: "b88#11", verdict: "SHARPENED", basis: "dual-face (the family's twenty-sixth attempt — the terminal agent's own, one batch after the orchestrator's three): the shell-act face — the derivation tests' first authentication piped through tail — is booked; re-issued pipeless in the same breath, the direct exit code the only verdict. Booked: the shell-act face only" },
  { key: "b89#0", verdict: "SHARPENED", basis: "dual-face (the exit-code-masking family's twenty-seventh attempt — the orchestrator's own opening of the platform's dedicated visit, one visit after the 23rd-26th cluster): the shell-act face — the visit's first ds typecheck and lint piped through tail with the code echoed from the pipeline's last stage — is booked; both gates were green and the redirect-to-file + direct-code pattern held for every later gate. Booked: the shell-act face only" },
  { key: "b89#1", verdict: "HELD", basis: "act face: the persistent shell's cwd carried a prior cd into a sibling repo — the gate ran green on the wrong tree and the banner's repo name caught it; which directory a shell sits in is author-side state no scheduled gate audits, the absolute-cd discipline the only enforcement" },
  { key: "b89#2", verdict: "HELD", basis: "act face: the compound inventory chain broke at grep's legitimate no-match exit — a chain's shape is the question's shape, and no scheduled gate audits command construction; re-issued tolerant in the same breath" },
  { key: "b89#3", verdict: "HELD", basis: "act face: the wrong-cwd git ls-files returned a false zero — a read-only probe against the wrong base reads like an audit finding; no scheduled gate audits probe provenance, the from-root re-issue the correction" },
  { key: "b89#4", verdict: "HELD", basis: "doc act face: a drafted nav link escaped the repository root — caught in self-review before any gate ran; doc-link targets are author-side, no scheduled gate compiles markdown hrefs" },
  { key: "b89#5", verdict: "HELD", basis: "the count-drift family's delivery-act face: per-file line counts staled by parallel siblings' in-flight edits within minutes — withdrawn to a structure-only tree with the closeout re-syncing from the final full-suite run; a doc draft's freshness against its own writers is author-side sequencing no scheduled gate audits" },
  { key: "b89#9", verdict: "HELD", basis: "coordination act face: the non-overlap map carried one mutable shared file in two agents' territories — the re-read-before-edit discipline held (concurrent appends coexisted without an overwrite); the serialization should have been named in the map itself, which no scheduled gate audits" },
  { key: "b89#11", verdict: "HELD", basis: "sibling-prose face: the atlas notes' retired numbers rode prose no scheduled gate parses against the sibling's live state — corrected at the citing tree with the volatile counts handed to the platform's own machine audit; the recurrence guard is the sweep discipline (any conviction that rewrites a shared file's content owes a workspace needle sweep), author-side" },
  { key: "b89#13", verdict: "HELD", basis: "channel-act face: a python heredoc through the shell edited a repo code file — the banned channel's next canonical act, content landing clean; no scheduled gate audits which channel wrote a file's bytes, the Edit-tool discipline the only enforcement" },
  { key: "b89#14", verdict: "HELD", basis: "pre-machine face: escape-level draft slips that crashed the generator on its own first run — the crash loop IS the factory check here (no scheduled gate renders diagrams); the shipped set carries its own machine self-check (zero missing-glyph warnings, zero box overflows) inside the generator itself" },
  { key: "b91#0", verdict: "HELD", basis: "acceptance face: the zero-glyph claim contradicted the machine's own output — the generator's glyph gate now exists as an exit-code check wired at npm run diagrams, but no scheduled gate runs the render; the acceptance-claims-against-the-machine discipline is author-side until a scheduled render gate exists" },
  ...
  { key: "b95#0", verdict: "HELD", basis: "shell-act face: the orchestrator's lint pipe printed the summary while eating the verdict — re-issued pipeless against a log file; the family's twenty-ninth sighting, no scheduled gate inspects the shell" },
  { key: "b95#1", verdict: "HELD", basis: "sequencing face: typecheck after tests left four TS2532s invisible to the green run — tsx's runtime silence is why the gate exists; gates before tests in every acceptance round" },
  { key: "b95#2", verdict: "HELD", basis: "expectation face: the invented method and memory-guessed digits were corrected against the captured golden — a test's expected values come from captures and reads, never from recall" },
  { key: "b95#3", verdict: "HELD", basis: "measurement face: single-sample verdicts inverted on re-run — re-measured as medians with counter-order; no gate commands the author's statistics" },
  { key: "b95#4", verdict: "HELD", basis: "draft face: per-frame listeners died before their frames arrived — single-collector buffering is the shape; the flake never reached a shipped test" },
  { key: "b95#5", verdict: "HELD", basis: "draft face: the condition field was read from imagination, not the type — the rule could never fire; the corrected case drives the real state shape" },
  { key: "b95#6", verdict: "HELD", basis: "measurement face: directory-form run, JIT-dead metric, and transient attribution — all three corrected before any verdict shipped; the platform's full-suite closeout confirmed the transient reading" },
  { key: "b95#7", verdict: "HELD", basis: "associativity face: the product hoist was caught at design review and downgraded to subexpression hoists — the frozen digits never moved; the near-miss is the wave's central hazard, booked" },
  { key: "b95#8", verdict: "HELD", basis: "baseline face: the render-face wall-clock was never captured, the beneficiary reported as unmeasured — honesty at the claim's face" },
  { key: "b95#9", verdict: "HELD", basis: "channel-act face: the node -e line edit violated the sanctioned-channel law on correct content — clean does not absolve" },
  { key: "b95#10", verdict: "HELD", basis: "control-flow face: the return-to-assignment conversion dropped the implicit break; self-caught before the mutation census could — the smoke test now asserts construction, not identity" },
  { key: "b95#11", verdict: "HELD", basis: "draft face: the corner mapping was convicted by the T3 statistics gate on its first run — the invariant did the checking the draft lacked" },
  { key: "b95#12", verdict: "HELD", basis: "aliasing face: the one-sided swap NaN'd the iteration and the existing pin fired — the isolation diff and full suite re-ran green" },
  { key: "b95#13", verdict: "HELD", basis: "sequencing face: the baseline overlapped its own edits and certified nothing — harmless only by the wave's bit-identity discipline" },
  { key: "b95#14", verdict: "HELD", basis: "baseline face: an unidentified concurrent write poisoned the Read baseline — git-diff-against-HEAD is the first act on any file in a concurrent wave; rebuilt from HEAD, 57/57" },
  { key: "b95#15", verdict: "HELD", basis: "cache face: the family-blind memo key served esc18's value to k3 and the W-E witness convicted at maxdev 4.9e-2 — the family name rides the key" },
  { key: "b95#16", verdict: "HELD", basis: "channel face: the python heredoc and node script replacements violated the law quoted in the wave's own prompts — mechanical renames of safe text still ride the Edit tool" },
  { key: "b95#17", verdict: "HELD", basis: "draft face: the off-by-one exponent was caught by the 30-digit frozen pin on its first run — the pin worked exactly as designed" },
  { key: "b95#18", verdict: "HELD", basis: "measurement face: paper-complexity reasoning lost to an A/B on the real hotspot — BigInt complexity is measured, not estimated" },
  { key: "b95#19", verdict: "HELD", basis: "shell-act face: the audit's own probe wore a pipe — self-caught in the read-only lane before any verdict was taken from it" },
  { key: "b95#20", verdict: "HELD", basis: "rule face: five agents read the rules after the writing, and one fallback branch kept a deleted scope's bindings until self-review — the rule surface is read first" },
  { key: "b96#0", verdict: "HELD", basis: "shell-act face: the orchestrator's prettier pipe printed PRETTIER=0 over a failing run — re-issued pipeless write-then-check; the family's thirtieth sighting, no scheduled gate inspects the shell" },
  { key: "b96#1", verdict: "HELD", basis: "channel face: three script-channel edits (a python heredoc on docs, a python -c on the census description, a second heredoc on the genealogy tiers) — content assert-guarded and correct, the channel violation stands where clean does not absolve" },
  { key: "b96#2", verdict: "HELD", basis: "pre-flight face: a wrong relative path and two Edits before a Read — the read-state guard refused both, zero shipped; the checklist, not the recovery, is the control" },
  { key: "b96#3", verdict: "HELD", basis: "expectation face: the composer anchors (0.35 vs the true 0.15 bonus default; the message substring) were corrected from the implementation before the green was trusted" },
  { key: "b96#4", verdict: "HELD", basis: "API-recall face: Number.isInfinite does not exist — the test run caught it; surfaces are read, not remembered" },
  { key: "b96#5", verdict: "HELD", basis: "feasibility face: the hot-start overclaim was refuted by its own strict mode at the saturated S→agent edge — the design restructured to constructive-only guarantees in the same breath" },
  { key: "b96#6", verdict: "HELD", basis: "construction face: the incremental scenario was rebuilt from the ring structure (four cancellations counted honestly); both diagnostic scripts re-derived independently" },
  { key: "b96#7", verdict: "HELD", basis: "draft face: two first Writes carried placeholder residue — caught by post-write re-reads and rewritten whole before any test executed them" },
  { key: "b96#8", verdict: "HELD", basis: "structural-Edit face: the post-Edit re-Read caught the decode body pushed out of its loop before any run" },
  { key: "b96#9", verdict: "HELD", basis: "anchor face: the 8/15 ratio, the f=0.1 cross terms, and the cannot-fail conditional were all self-convicted and re-derived; the tautology deleted on sight" },
  { key: "b96#10", verdict: "HELD", basis: "boundary face: the f=0 log→exp dust was answered with an exact direct-addition path, the strict equality kept" },
  { key: "b96#11", verdict: "HELD", basis: "shell-act face: EXIT=0 over a real exit 2 — self-caught the same turn and re-read from a redirect; sibling noise makes the masked verdict misleading" },
  { key: "b96#12", verdict: "HELD", basis: "distributional-anchor face: the quantile misestimate was eliminated by construction — a no-probation counter-proof now pins that admission rides the probation path" },
  { key: "b96#13", verdict: "HELD", basis: "constitution face: four agents' piped verdicts (C/D/E/H) were all self-caught and re-run under pipefail — the law was in the wave's own prompts; enforcement stays with the actor's re-run" },
  { key: "b96#14", verdict: "HELD", basis: "pre-run face: mojibake identifier and closure-API mismatch fixed by self-review before the first execution" },
  { key: "b96#15", verdict: "HELD", basis: "attribution face: three refuted judgments were withdrawn with the evidence (anchors.ts read, the registered-divergence confirmed, the two-path constant honored) before any implementation was approved" },
  { key: "b96#16", verdict: "HELD", basis: "family face: md5 and registry reads replaced the by-name attributions; the pin count and glob overreach corrected in the report" },
  { key: "b96#17", verdict: "HELD", basis: "territory face: the missed experiments/ directory was read in full after the test import exposed it; the coverage discount declared, not silent" },
  { key: "b96#18", verdict: "HELD", basis: "symmetric-withdrawal face: the scoping charge withdrawn after re-reading the block scopes; the stale-prose conviction stands enrolled as a finding" },
  { key: "b96#19", verdict: "HELD", basis: "predicate face: the direction was rewritten from the inequality and the sentinel died with the draft" },
  { key: "b96#20", verdict: "HELD", basis: "dual-path face: the potential anchor was machine-corrected before it became the expectation — the shipped test pins the verified value" },
  { key: "b96#21", verdict: "HELD", basis: "conjugation-site face: the double conjugation was killed by the module's own PSD assertion and analytic anchors — the detector shipped with the module" },
  { key: "b96#22", verdict: "HELD", basis: "disclosure face: the two-tier damping report and the whitelist deviations were disclosed with the act, not discovered later" },
  { key: "b96#23", verdict: "HELD", basis: "constant face: all four recalled digits were test-convicted and corrected from the tables they live in before ship" },
  { key: "b96#24", verdict: "HELD", basis: "transcript face: the adversary direction bugs were caught by the invariant sweep; the slow probe replaced by a better criterion" },
  ...
  // ---- batch 100 (R18 second innovation wave) — born-audited at burial time, every row held ----
  { key: "b100#0", verdict: "HELD", basis: "instance face: instances are premise-checked in the module's own test battery — the lesson is process, the gate is the author's discipline" },
  { key: "b100#1", verdict: "HELD", basis: "validation-order face: the module's guard order is documented where the guards live — walking it on paper is author practice" },
  { key: "b100#2", verdict: "HELD", basis: "partition face: the random-family coverage is pinned by construction in the delivered tests — the dropped tail was caught by the same family's own audit" },
  { key: "b100#3", verdict: "HELD", basis: "argument-contract face: strict typing convicts shape at compile — the row is the test-author's, booked on the practice" },
  { key: "b100#4", verdict: "HELD", basis: "event-order face: the hook order is the module's own contract, pinned by its tests — the row books the test-side race" },
  { key: "b100#5", verdict: "HELD", basis: "exit-predicate face: the delivered termination lattice (Illinois flip, two-sided flat, stagnation midpoint) is pinned test-by-test in r18a — the lesson is design-order" },
  { key: "b100#6", verdict: "HELD", basis: "dead-state face: lint is the gate and it fired — the confession rides with the catch" },
  { key: "b100#7", verdict: "HELD", basis: "memory-shape face: the pending-dual-source mark is the enforced shape for every new module header — the draft never reached an artifact" },
  { key: "b100#8", verdict: "HELD", basis: "portability face: workspace-local probes are the rule; the failed probe cost one round-trip, nothing shipped" },
  { key: "b100#9", verdict: "HELD", basis: "quadrature face: the closed form is pinned against Monte Carlo and finite-sum legs in the delivered tests — the red was the harness's own numerics" },
  { key: "b100#10", verdict: "HELD", basis: "identifier-space face: the bidirectional-consistency diagnostic is delivered beside the fix — held on author practice" },
  { key: "b100#11", verdict: "HELD", basis: "owner-matched-deletion face: the internal verifier that killed the ghost edge ships with the module — held on author practice" },
  { key: "b100#12", verdict: "HELD", basis: "control-domain face: the delivered negative controls sit inside the validated domain — the vacuous draft died at self-review" },
  { key: "b100#13", verdict: "HELD", basis: "reachability face: repositioned as an enumeration proof in the delivered test — held on author practice" },
  { key: "b100#14", verdict: "HELD", basis: "narrow face: the no-unsafe-* rules are the standing gate and they fired — booked as the family's recurrence sighting" },
  { key: "b100#15", verdict: "HELD", basis: "re-derivation face: caught at re-derivation before any red — held on author practice" },
  { key: "b100#16", verdict: "HELD", basis: "guarantee-shape face: the four-state conservation and the bound are exactly what the delivered tests pin — the deadline draft asked a promise the method never makes" },
  { key: "b100#17", verdict: "HELD", basis: "explicit-kernel face: the delivered endpoint tests state their kernel — held on author practice" },
  { key: "b100#18", verdict: "HELD", basis: "draft-death face: lint fired on the draft — the convergence round is the record" },
  { key: "b100#19", verdict: "HELD", basis: "sibling-state face: the terminal whole-project typecheck is the closeout's gate and it closed at zero — the row books the mid-wave reading discipline" },
  { key: "b100#20", verdict: "HELD", basis: "bracket-arm face: the delivered certificate pins the bracket that matters — the comparison draft was test-side" },
  { key: "b100#21", verdict: "HELD", basis: "red-ink face: hand arithmetic is unconfirmed until the machine confirms — held on author practice" },
  { key: "b100#22", verdict: "HELD", basis: "role-label face: the delivered certificate asserts the bracket with named arms — the swap draft never reached a run" },
  { key: "b100#23", verdict: "HELD", basis: "single-writer face: the compiler is the gate and it fired" },
  { key: "b100#24", verdict: "HELD", basis: "strict-face re-read: typecheck is the standing gate — convergence rounds are booked, not hidden" },
  { key: "b100#25", verdict: "HELD", basis: "habit face: static-import-only is the enforced module shape — the lean was excluded pre-ship, the disposition books the habit" },
  { key: "b100#26", verdict: "HELD", basis: "checklist face: the report's own verification block is the counter-evidence — held on author practice" },
  { key: "b100#27", verdict: "HELD", basis: "probe-first face: exit-1 probes cost one round-trip — held on author practice" },
  { key: "b100#28", verdict: "HELD", basis: "verify-at-hash face: the family register's own hash columns are the machine check — the assumption was corrected against them" },
  { key: "b100#29", verdict: "HELD", basis: "declared-partial face: the report declares its read extent — the law this row polices is the declaration itself" },
  { key: "b100#30", verdict: "HELD", basis: "declared-single-round face: seeded RNG is the standing reproducibility gate — the declaration is the record" },
  { key: "b100#31", verdict: "HELD", basis: "archive-existence face: the absence is itself booked as a finding — held on author practice" },
  { key: "b100#32", verdict: "HELD", basis: "member-wise face: the family register's hash columns convicted the remembered hash, not the lock — machine-overruled in the wild" },
  { key: "b100#33", verdict: "HELD", basis: "timeout-provenance face: held on author practice — the suite's own duration is the counter-evidence" },
  { key: "b100#34", verdict: "HELD", basis: "dual-form face: the delivered ripple analysis ran both forms — held on author practice" },
  { key: "b100#35", verdict: "HELD", basis: "withdrawal face: held on author practice — withdrawals are booked as sharply as convictions" },
  { key: "b100#36", verdict: "HELD", basis: "quantifier-walk face: the delivered theorem states lambda_max(B) with a POVM bound over all CPTP — the generalization was corrected before it shipped" },
  { key: "b100#37", verdict: "HELD", basis: "register-over-recall face: the dual-source register is the standing citation gate — the correction happened at the register" },
  { key: "b100#38", verdict: "HELD", basis: "complete-block face: held on author practice — the re-run produced the complete block" },
  { key: "b100#39", verdict: "HELD", basis: "per-repo-convention face: the machine's runner output is the counter-evidence — held on author practice" },
  { key: "b100#40", verdict: "HELD", basis: "pipe-shape face: exit-code honesty was honored by complete-block reads — the shape is booked as the family's sighting" },
  { key: "b100#41", verdict: "HELD", basis: "declared-authority face: W-4 and the DECLARED constants are the standing gates — the inference was corrected against them" },
  { key: "b100#42", verdict: "HELD", basis: "bounded-segment face: the reconciliation against burial's own 814 is the machine check — held on author practice" },
  { key: "b100#43", verdict: "HELD", basis: "quantization face: the delivered wiring tests assert at the platform's own rounding — the tolerance is derived, not chosen" },
  { key: "b100#44", verdict: "HELD", basis: "guarantee-vs-observation face: the delivered test says which sentence it asserts — held on author practice" },
  { key: "b100#45", verdict: "HELD", basis: "wire-shape face: the validator ships holding the string width — the compiler conviction is the record" },
  { key: "b100#46", verdict: "HELD", basis: "surgical-format face: format:check over the whole repo is the standing gate and it closed — held on author practice" },
  { key: "b100#47", verdict: "HELD", basis: "band-invariant face: the delivered constructor validator is the gate — it rejected the illegal ladder (the accident self-proved the gate)" },
  { key: "b100#48", verdict: "HELD", basis: "lesson-uptake face: the no-unsafe-* gate fired on the recurrence — booked as the family's same-wave sighting" },
  { key: "b100#49", verdict: "HELD", basis: "replace-not-decorate face: the compiler is the gate and the dead code died with the approach" },
  { key: "b100#50", verdict: "HELD", basis: "hand-repair face: format:check closed over the whole file — held on author practice" },
  { key: "b100#51", verdict: "HELD", basis: "behavior-first face: the delivered conviction tests open on behavior — held on author practice" },
  { key: "b100#52", verdict: "HELD", basis: "sanity-face-first law: the module's own QV_DEGENERATE guard is the standing gate — it convicted the fixture, the source was correct" },
  { key: "b100#53", verdict: "HELD", basis: "load-at-birth face: esbuild is the gate and it refused at zero cost" },
  { key: "b100#54", verdict: "HELD", basis: "accessor face: the delivered census/pairing/smuggling faces pin the correct lookups — the first red exposed both, the tests now hold them" },
  { key: "b100#55", verdict: "HELD", basis: "value-claim face: the delivered test compares element-wise — held on author practice" },
  { key: "b100#56", verdict: "HELD", basis: "explicit-expansion face: the delivered monotonicity test asserts the expanded direction across 32,505 steps — the expansion is now the practice" },
  { key: "b100#57", verdict: "HELD", basis: "float-realizability face: the delivered test asserts two-path consistency on the tiny round — 1e-33 is not 0 and the test says so" },
  { key: "b100#58", verdict: "HELD", basis: "no-scratch-exemption face: the sanctioned-channel family's standing row — the relapse within one visit is its own row, both files deleted" },
  { key: "b100#59", verdict: "HELD", basis: "policy-divergence face: typecheck and lint are the standing gates — both closed, the rounds are the record" },
  { key: "b100#60", verdict: "HELD", basis: "handover-scope face: the declared warranty boundary is auditable in the delivered report — held as the boundary itself" },
  { key: "b100#61", verdict: "HELD", basis: "pin-strength face: the delivered showcase asserts at M=3000 fixed-seed — held on author practice" },
  { key: "b100#62", verdict: "HELD", basis: "scientific-notation face: the delivered showcase prints scientific magnitudes — held on author practice" },
  { key: "b100#63", verdict: "HELD", basis: "pattern-question face: the re-derived pattern found every badge — held on author practice" },
  { key: "b100#64", verdict: "HELD", basis: "live-file face: the doc-truth inventory's whole method is live-file verification — the row books the near-inheritance" },
  { key: "b100#65", verdict: "HELD", basis: "disclosed-intermediate face: the closeout's full run is the number the docs carry — the disclosure is the record" },
  { key: "b100#66", verdict: "HELD", basis: "no-post-hoc-loosening face: the assertion stood as run and passed — held on author practice" },
  { key: "b100#67", verdict: "HELD", basis: "read-the-traps face: the delivered segment prints the bracket that pins the true minimum — reading booked traps is now the practice" },
  { key: "b100#68", verdict: "HELD", basis: "unpiped-verdict face: the FETCH_REAL re-check is the standing ritual — the pipe shape is the family's 31st sighting, booked with its catch" },
  { key: "b100#69", verdict: "HELD", basis: "landed-only face: the changelog was rewritten against the machine-verified delivery — a ledger records what has landed" },
  { key: "b100#70", verdict: "HELD", basis: "executed-arithmetic face: the number now in the doc was computed before any gate ran — doc arithmetic is executed, not recalled" },
  { key: "b100#71", verdict: "HELD", basis: "no-convenience-exemption face: the diff was verified exactly one line and the edit stood correct — the channel, not the content, is the conviction; the sanctioned-channel family's standing row" },

  // ---- batch 101 (R18 reinforcement batch) — born-audited at burial time, every row held ----
  { key: "b101#0", verdict: "HELD", basis: "literal-derivation face: the module's own irrational-overlap guard convicted the constant at birth — held on author practice" },
  { key: "b101#1", verdict: "HELD", basis: "algebraic-identity face: the norm assertion is the standing gate and it convicted at 4≠6" },
  { key: "b101#2", verdict: "HELD", basis: "lint face: the rule fired — the convergence rides with the delivery" },
  { key: "b101#3", verdict: "HELD", basis: "quantifier-exactness face: the per-instance scatter is delivered as honest data beside the global theorem — the machine's mouth is the report's" },
  { key: "b101#4", verdict: "HELD", basis: "guard-is-the-law face: the read-state guard refused the stale edit and the re-issued one landed — the refusal is the record" },
  { key: "b101#5", verdict: "HELD", basis: "slash-star face: the compiler is the gate — the cheapest syntax red there is" },
  { key: "b101#6", verdict: "HELD", basis: "unit-naming face: held on author practice — units are named at the assertion" },
  { key: "b101#7", verdict: "HELD", basis: "degenerate-input face: the delivered pre-check names the empty intersection — held on author practice" },
  { key: "b101#8", verdict: "HELD", basis: "last-line-copy face: the delivered threshold is the derivation's 0.99 — held on author practice" },
  { key: "b101#9", verdict: "HELD", basis: "walk-forward face: the delivered recurrence is pinned by exact-deviation tests — the machine conviction is the record" },
  { key: "b101#10", verdict: "HELD", basis: "spec-is-hypothesis face: the delivered theorem states the hard-cascade domain with its exact boundary — the false global claim died in the implementation, which is where spec claims go to be tested" },
  { key: "b101#11", verdict: "HELD", basis: "correct-result-illegal-channel face: the sanctioned-channel family's standing row — the result was re-achieved through the Edit channel" },
  { key: "b101#12", verdict: "HELD", basis: "sign-structure face: the delivered deepStrictEqual cross-check is the gate that convicted the absolutized roots" },
  { key: "b101#13", verdict: "HELD", basis: "smallest-k face: the delivered k=3 witness is the standing assertion — k=2's symmetry is the camouflage the test now refuses to trust" },
  { key: "b101#14", verdict: "HELD", basis: "illegal-direction face: the delivered control asserts the illegal direction explicitly — held on author practice" },
  { key: "b101#15", verdict: "HELD", basis: "residue face: both drafts healed pre-gate — the booking is the habit's evidence" },
  { key: "b101#16", verdict: "HELD", basis: "full-suite-before-shape face: the repo's own no-orphan-modules guard is the standing gate — the re-homed kernel passes it and the unlisted guard is now a ripple-map entry" },
  { key: "b101#17", verdict: "HELD", basis: "test-question face: the module was never wrong — held on the test side's correction" },
  { key: "b101#18", verdict: "HELD", basis: "endpoint-check face: the delivered certificate does not depend on the refuted claim, and the theorem's wording now matches the machine's testimony — the endpoint check is the practice" },
  { key: "b101#19", verdict: "HELD", basis: "game-rules-from-invariant face: the delivered LEG C conserves mass with a unique champion — the invariants tests are the standing gate" },
  { key: "b101#20", verdict: "HELD", basis: "budget-depth-agreement face: the delivered control sets budget 0 and asserts depth 0 — the agreement is now structural" },
  { key: "b101#21", verdict: "HELD", basis: "measured-budget face: the tiering and its cost boundary are recorded in the delivered header and README — performance is part of the usability claim" },
  { key: "b101#22", verdict: "HELD", basis: "b96#13 family face: the terminal gate runs capture $? first — the standing ritual, self-applied" },
  { key: "b101#23", verdict: "HELD", basis: "self-made-artifact face: machine output now flows through the harness's own logging — the boundary case is booked as the record" },
  { key: "b101#24", verdict: "HELD", basis: "representation-is-not-value face: the delivered assertion compares through fCmp — held on author practice" },
  { key: "b101#25", verdict: "HELD", basis: "repo-policy face: typecheck and test were re-verified after the lint convergence — the rounds are the record" },
  { key: "b101#26", verdict: "HELD", basis: "local-slope face: the delivered control's amplitude, grid-blindness and wing-conviction are all probed and pinned — the dip now convicts, the grid stays provably blind" },
  { key: "b101#27", verdict: "HELD", basis: "no-convenience-exemption face: the append stood correct in content and was redone through the Edit channel after the read-state guard refused the first stale attempt — the lineage is the standing row" },

  // ---- batch 102 (R19 third innovation wave) — born-audited at burial time, every row held ----
  { key: "b102#0", verdict: "HELD", basis: "small-edit camouflage face: both edits re-landed through Edit — the family's standing row" },
  { key: "b102#1", verdict: "HELD", basis: "conjugation-map face: the index map is now pinned in the delivered test comments — held on author practice" },
  { key: "b102#2", verdict: "HELD", basis: "representable-randomness face: the delivered 32-bit hi/lo form is pinned; the latent defect it surfaced is booked as a report" },
  { key: "b102#3", verdict: "HELD", basis: "which-optimum face: the delivered assertion names its optimum in closed form — held on author practice" },
  { key: "b102#4", verdict: "HELD", basis: "spec-is-hypothesis face: the delivered theorem states sup-uniqueness with the complete hole enumeration — the machine's mouth" },
  { key: "b102#5", verdict: "HELD", basis: "spec-digit face: loading semantics disclosed in the delivered README; the exactness claim restated as its theorem path" },
  { key: "b102#6", verdict: "HELD", basis: "shape-is-the-violation face: zero bytes, booked anyway — the family's standing shape row" },
  { key: "b102#7", verdict: "HELD", basis: "draft-death face: the shipped files grep-clean, gates double-green — the residue died at self-review" },
  { key: "b102#8", verdict: "HELD", basis: "kernel-not-fingers face: all three values fixed to the machine's — the lesson is the booking" },
  { key: "b102#9", verdict: "HELD", basis: "method-floor face: the delivered dual-path uses the linear form at 1e-12 with the solver cross-check at its documented floor" },
  { key: "b102#10", verdict: "HELD", basis: "two-gates face: the delivered typed helper satisfies both — held on author practice" },
  { key: "b102#11", verdict: "HELD", basis: "closed-loop face: the final git-status check caught it — the footprint check is the standing gate" },
  { key: "b102#12", verdict: "HELD", basis: "vacuous-census face: the delivered census carries exact per-input alignments — the surface is genuinely swept" },
  { key: "b102#13", verdict: "HELD", basis: "cross-term face: the delivered bilinear is pinned by the vanish test that convicted the draft" },
  { key: "b102#14", verdict: "HELD", basis: "factored-denominator face: the delivered arithmetic is pinned by the first-red test" },
  { key: "b102#15", verdict: "HELD", basis: "cardinality-check face: the module's own named rejection is the standing gate — it convicted the fixture" },
  { key: "b102#16", verdict: "HELD", basis: "convergence-round face: the gate caught both — booked with the delivery" },
  { key: "b102#17", verdict: "HELD", basis: "no-self-made-artifacts face: all gates re-run with PIPESTATUS, no landing — the report's lines come from the re-runs" },
  { key: "b102#18", verdict: "HELD", basis: "shape face: the family's standing no-payload row" },
  { key: "b102#19", verdict: "HELD", basis: "scoped-format face: format:check closed over own files only" },
  { key: "b102#20", verdict: "HELD", basis: "guard-replay face: the read-state guard refused the stale follow-up — the system worked as designed; the violation stands booked" },
  { key: "b102#21", verdict: "HELD", basis: "premise-verification face: the bump went 0.4.0->0.5.0 against the manifest and the commit — the premise is now verified at the source" },
  { key: "b102#22", verdict: "HELD", basis: "intuition-is-a-formula face: the delivered 8-state parity chain carries the exact solution with the engine within 0.49 sigma" },
  { key: "b102#23", verdict: "HELD", basis: "load-bearing-imaginary face: pinned by idempotence at 1e-15 and recorded in the test's name" },
  { key: "b102#24", verdict: "HELD", basis: "double-orientation face: the delivered double-orientation probe gives bitwise-identical traces through different code paths" },
  { key: "b102#25", verdict: "HELD", basis: "self-review face: cleared before the gates — the booking is the habit's evidence" },
  { key: "b102#26", verdict: "HELD", basis: "never-twice face: caught at self-review before any run — the class's R7 history is why the eye knew to look" },
  { key: "b102#27", verdict: "HELD", basis: "shape-assert face: held on author practice — the probe's red is the cheapest red" },
  { key: "b102#28", verdict: "HELD", basis: "NaN-is-a-question face: the method was sound; the probe's coefficient count was the bug" },
  { key: "b102#29", verdict: "HELD", basis: "uptake-failure face: the delivered comparison uses three-way refinement — the lesson's second occurrence is the row's reason for being" },
  { key: "b102#30", verdict: "HELD", basis: "constrained-reference face: the delivered reference enumerates endpoint and periodic candidates — the module needed no defense" },
  { key: "b102#31", verdict: "HELD", basis: "no-single-snapshot face: the delivered control widens through unknown as designed; the verdict is read from a clean run" },
  { key: "b102#32", verdict: "HELD", basis: "$?-first face: the standing ritual applied after the first slip" },
  { key: "b102#33", verdict: "HELD", basis: "label-reread face: caught pre-commit, no assertion affected" },
  { key: "b102#34", verdict: "HELD", basis: "fixture-from-definition face: the delivered fixture carries three genuinely non-monotone segments" },
  { key: "b102#35", verdict: "HELD", basis: "built-fork face: the delivered fork is built from equal initial slopes" },
  { key: "b102#36", verdict: "HELD", basis: "scaling-law face: the delivered control's conviction ratio is 12.7x — the certificate's scaling law read first" },
  { key: "b102#37", verdict: "HELD", basis: "rare direction face: the module now named-rejects the infinite-Lipschitz grid — a test-side first red that was the module's fault, fixed and boundary-upgraded" },
  { key: "b102#38", verdict: "HELD", basis: "flat-step face: the delivered test asserts sigma(0)=0 on the flat step — the mathematics is one" },
  { key: "b102#39", verdict: "HELD", basis: "domain-from-module face: removed from the bad list — the contract read from the module" },
  { key: "b102#40", verdict: "HELD", basis: "per-state face: the delivered assertion splits by verdict" },
  { key: "b102#41", verdict: "HELD", basis: "check-before-feed face: the delivered loop checks decision==='continue' — the refusal is the mechanism" },
  { key: "b102#42", verdict: "HELD", basis: "cancelled-is-not-green face: the delivered tests use the options form — a cancelled test is not a green test" },
  { key: "b102#43", verdict: "HELD", basis: "mechanism's-lattice face: the delivered lattice registers exactly the six real cells" },
  { key: "b102#44", verdict: "HELD", basis: "loader-first face: the loader is the first gate" },
  { key: "b102#45", verdict: "HELD", basis: "strict-face reread: both strict faces re-read and satisfied" },
  { key: "b102#46", verdict: "HELD", basis: "cascade-root face: rebuilt on the unknown-view convention, dead branches deleted — one wrong narrow was forty findings" },
  { key: "b102#47", verdict: "HELD", basis: "recognition-is-not-immunity face: the delivered fixture renames the protocol field and pins the collision in a comment" },
  { key: "b102#48", verdict: "HELD", basis: "never-convict face: the delivered 429 test carries an explicit poll counter and the length assertion that killed the false pass" },
  { key: "b102#49", verdict: "HELD", basis: "loader-catches face: the loader caught what the eye carried past" },
  { key: "b102#50", verdict: "HELD", basis: "test's-question face: the validator was innocent; the input was normalized" },
  { key: "b102#51", verdict: "HELD", basis: "deletion-reparse face: the deletion was re-parsed and re-landed" },
  { key: "b102#52", verdict: "HELD", basis: "boundary-case face: switched to grep on the spot — the strict reading is honored by the booking itself" },
  { key: "b102#53", verdict: "HELD", basis: "platform-TS face: explicit fields plus constructor-body assignment — the delivered file passes the face" },
  { key: "b102#54", verdict: "HELD", basis: "provenance face: the constants were chosen before the run and the disclosure states so — design, not curve-fitting" },
  { key: "b102#55", verdict: "HELD", basis: "formula-on-instance face: the delivered expectation comes from the formula on the instance" },
  { key: "b102#56", verdict: "HELD", basis: "ghost-expression face: killed at self-review before implementation" },
  { key: "b102#57", verdict: "HELD", basis: "write-time catch face: caught at write-time, replaced with fresh-id injection" },
  { key: "b102#58", verdict: "HELD", basis: "corner face: the corner is preserved and pinned — bit-identity obligations cover corners" },
  { key: "b102#59", verdict: "HELD", basis: "gate's catch: optional-chained in the delivered file" },
  { key: "b102#60", verdict: "HELD", basis: "closeout-habitat face: zero bytes landed (the script's own MISS guard exited first) and every sync went through Edit — the structural lesson: the closeout checklist now carries the Edit-only law at the bulk-sync step" },

  // ---- batch 103 (R20 fourth innovation wave) — born-audited at burial time, every row held ----
  { key: "b103#0", verdict: "HELD", basis: "channel face: reverted and re-landed via Edit — the standing row" },
  { key: "b103#1", verdict: "HELD", basis: "comment-prose face: the compiler refused both — the cheapest reviewer of prose" },
  { key: "b103#2", verdict: "HELD", basis: "stdlib face: caught at birth by the compiler" },
  { key: "b103#3", verdict: "HELD", basis: "index-bound face: caught at first run" },
  { key: "b103#4", verdict: "HELD", basis: "sign-test face: corrected at first red" },
  { key: "b103#5", verdict: "HELD", basis: "certifiable-semantics face: the machine decides what an anchor can certify — the stricter discipline adopted" },
  { key: "b103#6", verdict: "HELD", basis: "machine-tally face: the hand count is a hypothesis" },
  { key: "b103#7", verdict: "HELD", basis: "scratch-target face: booked as-is, no rollback laundering" },
  { key: "b103#8", verdict: "HELD", basis: "attempt-is-sighting face: stopped before execution" },
  { key: "b103#9", verdict: "HELD", basis: "derived-not-drafted face: both rewritten with convictions recorded" },
  { key: "b103#10", verdict: "HELD", basis: "spec-is-hypothesis face: the refined condition is the deliverable" },
  { key: "b103#11", verdict: "HELD", basis: "expectation face: amplitude zero exact, visibility a measured scale" },
  { key: "b103#12", verdict: "HELD", basis: "verification-scripts-are-evidence face: caught before any deliverable carried it" },
  { key: "b103#13", verdict: "HELD", basis: "temp-file face: deletion is remediation, not absolution" },
  { key: "b103#14", verdict: "HELD", basis: "ritual face: applied after the slip" },
  { key: "b103#15", verdict: "HELD", basis: "shape face: died at the first assert" },
  { key: "b103#16", verdict: "HELD", basis: "computed-not-guessed face: the machine's quadrant stands" },
  { key: "b103#17", verdict: "HELD", basis: "writer-rereads face: caught pre-gate" },
  { key: "b103#18", verdict: "HELD", basis: "red-authored-first face: booked though the outcome was honest" },
  { key: "b103#19", verdict: "HELD", basis: "pre-table face: the machine's verdicts pinned as the permanent record" },
  { key: "b103#20", verdict: "HELD", basis: "convergence face: booked with the delivery" },
  { key: "b103#21", verdict: "HELD", basis: "literal-fitted-and-convicted face: the real theorem delivered with the refutation named in tests" },
  { key: "b103#22", verdict: "HELD", basis: "engine-check face: aligned at the third attempt" },
  { key: "b103#23", verdict: "HELD", basis: "test-question face: the module was innocent" },
  { key: "b103#24", verdict: "HELD", basis: "provenance face: derived and commented at the tolerance" },
  { key: "b103#25", verdict: "HELD", basis: "repo-law face: closed error-code set reused, run closed green" },
  { key: "b103#26", verdict: "HELD", basis: "uptake-failure face: the same-wave recurrence is the row's reason" },
  { key: "b103#27", verdict: "HELD", basis: "forgery-path face: the hazard became the negative control" },
  { key: "b103#28", verdict: "HELD", basis: "ritual face: applied after the slip" },
  { key: "b103#29", verdict: "HELD", basis: "verify-first face: the errors died in the probe and the fixes carried into the module" },
  { key: "b103#30", verdict: "HELD", basis: "modality-literal face: the corrected modality delivered" },
  { key: "b103#31", verdict: "HELD", basis: "off-the-hiding-family face: the cross term is now asserted" },
  { key: "b103#32", verdict: "HELD", basis: "remembered-formula face: the re-derivation is the deliverable" },
  { key: "b103#33", verdict: "HELD", basis: "ordering face: the overturn became the theorem" },
  { key: "b103#34", verdict: "HELD", basis: "boundary-widened face: another intuition convicted" },
  { key: "b103#35", verdict: "HELD", basis: "own-machinery face: multiset semantics fixed, census clean on the third run" },
  { key: "b103#36", verdict: "HELD", basis: "tuple-policy face: classified fixes, both green" },
  { key: "b103#37", verdict: "HELD", basis: "controls-bite face: all three tightened" },
  { key: "b103#38", verdict: "HELD", basis: "channel face: the sixth sighting — reverted and re-landed" },
  { key: "b103#39", verdict: "HELD", basis: "ritual face: both applied after the slips" },
  { key: "b103#40", verdict: "HELD", basis: "comment-prose face: the family's second same-wave sighting" },
  { key: "b103#41", verdict: "HELD", basis: "legal-traffic face: continued-fraction recovery re-derived" },
  { key: "b103#42", verdict: "HELD", basis: "re-entry face: guarded where they re-enter" },
  { key: "b103#43", verdict: "HELD", basis: "vertex face: the tie became the theorem" },
  { key: "b103#44", verdict: "HELD", basis: "both-directions face: the exposure clause is the missing half" },
  { key: "b103#45", verdict: "HELD", basis: "grid-scale face: dual-scale re-ran" },
  { key: "b103#46", verdict: "HELD", basis: "smallest-target face: zero sed lineage in the deliverable tree" },
  { key: "b103#47", verdict: "HELD", basis: "probes-die-young face: all three fixed before the module" },
  { key: "b103#48", verdict: "HELD", basis: "spec-formula face: the corrected law delivered, the original in perpetual trial" },
  { key: "b103#49", verdict: "HELD", basis: "pre-gate-interception face: the ripple grep did the gate's work before the gate — the pin sweep is now part of the delivery" },
  { key: "b103#50", verdict: "HELD", basis: "recount face: declared totals computed by the registry's own recount — the pre-booked slip enrolled" },

  // ---- batch 104 (R21 the design-stock clearance) — born-audited at burial time, every row held ----
  { key: "b104#0", verdict: "HELD", basis: "zero-shift face: the machine's no-shift is the theorem, the decomposition the deliverable" },
  { key: "b104#1", verdict: "HELD", basis: "TP-at-merge face: killed at construction" },
  { key: "b104#2", verdict: "HELD", basis: "solver-contract face: spectral shifting before trust" },
  { key: "b104#3", verdict: "HELD", basis: "convention-naming face: died at the cross-check" },
  { key: "b104#4", verdict: "HELD", basis: "first-red face: corrected to the mathematics" },
  { key: "b104#5", verdict: "HELD", basis: "wrong-idea-at-work face: the misreading became the control's fixture" },
  { key: "b104#6", verdict: "HELD", basis: "convergence face: booked with the delivery" },
  { key: "b104#7", verdict: "HELD", basis: "rollback-then-Edit face: the protocol executed exactly; Edit-lineage final state" },
  { key: "b104#8", verdict: "HELD", basis: "orientation-from-definition face: recorded in the test" },
  { key: "b104#9", verdict: "HELD", basis: "tested-primitives face: both substitutions made the batteries green" },
  { key: "b104#10", verdict: "HELD", basis: "drafted-law face: the true law pinned in module, README, and a convicting negative control" },
  { key: "b104#11", verdict: "HELD", basis: "core-spec face: the surviving exact theorem is the deliverable; the interlaced gap priced, not hidden" },
  { key: "b104#12", verdict: "HELD", basis: "ascending-bit-order face: the split died at zero" },
  { key: "b104#13", verdict: "HELD", basis: "directions-follow-the-machine face" },
  { key: "b104#14", verdict: "HELD", basis: "diff-in-same-breath face: the line restored" },
  { key: "b104#15", verdict: "HELD", basis: "guards-before-fixtures face: 69s to 663ms" },
  { key: "b104#16", verdict: "HELD", basis: "certificate-architecture face: sign-change certificates with a completeness face — hidden multiplicities disclosed, never guessed" },
  { key: "b104#17", verdict: "HELD", basis: "tolerance-from-isolation face: uniqueness + intermediacy with a wide band; forgeries still convict" },
  { key: "b104#18", verdict: "HELD", basis: "convergence face" },
  { key: "b104#19", verdict: "HELD", basis: "ritual face: re-verified unpiped" },
  { key: "b104#20", verdict: "HELD", basis: "latitude face: adjudicated accepted — the audit faces ARE the machinery; booked as latitude, not permission" },
  { key: "b104#21", verdict: "HELD", basis: "small-n-camouflage face: the cross path located what the vacuous green hid" },
  { key: "b104#22", verdict: "HELD", basis: "four-faces face: the eigenvalue algebra outranks the spec's face count; c4 exact in BigInt" },
  { key: "b104#23", verdict: "HELD", basis: "own-grid face: the spec's grid is the evidence against its claim" },
  { key: "b104#24", verdict: "HELD", basis: "refutation-byproduct face: beta-bar=0 at p=1/2 — a new theorem from a refuted form" },
  { key: "b104#25", verdict: "HELD", basis: "memoization + no-pipe face" },
  { key: "b104#26", verdict: "HELD", basis: "room-for-mathematics face: the discovery promoted to its own theorem" },
  { key: "b104#27", verdict: "HELD", basis: "three-route face: 6591 grid points at 1.7e-16 worst" },
  { key: "b104#28", verdict: "HELD", basis: "nothing-special-because-small face" },
  { key: "b104#29", verdict: "HELD", basis: "typecheck-before-test face" },
  { key: "b104#30", verdict: "HELD", basis: "convergence round face: five small faces, one row" },
  { key: "b104#31", verdict: "HELD", basis: "boundary face: the b102#17 standing row" },
  { key: "b104#32", verdict: "HELD", basis: "own-conduct-first face: the triage IS the coordination worklist" },
  { key: "b104#33", verdict: "HELD", basis: "habitat face: zero recurrence after the second catch" },
  { key: "b104#34", verdict: "HELD", basis: "group-cyclotype face: exact binary coefficients per group element" },
  { key: "b104#35", verdict: "HELD", basis: "provable-vs-priced face: the dual-ln certificate delivered, the algebraic remainder priced" },
  { key: "b104#36", verdict: "HELD", basis: "convicted-cell face: transcription checked against the sign-trace" },
  { key: "b104#37", verdict: "HELD", basis: "convergence face" },
  { key: "b104#38", verdict: "HELD", basis: "standing protocol face" },
  { key: "b104#39", verdict: "HELD", basis: "verified-before-carried face: the docs slip left to the docket" },
  { key: "b104#40", verdict: "HELD", basis: "grouping-law face: every face has both columns in the agent's ledger" },
];
...
export const FIRING_EVIDENCE: readonly FiringEvidence[] = [
  ...
  { key: "b85#35", path: "mutant-census/test/genealogy.test.ts", needle: "the derivation skeleton bites" },
];
