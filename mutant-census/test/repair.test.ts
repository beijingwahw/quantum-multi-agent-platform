...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), thirty-five sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 35);
  assert.equal(byVerdict("HELD"), 134);
  assert.equal(REPAIR_AUDIT.length, 180);
  ...
  assert.equal(gate, 252);
  assert.equal(booked, 169);
