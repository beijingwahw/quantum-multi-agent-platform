...
test("the repair state: twelve upgrades (b67#6, b70#3 and b78#13 flipped under R1), fifty-two sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 52);
  assert.equal(byVerdict("HELD"), 169);
  assert.equal(REPAIR_AUDIT.length, 233);
  ...
  assert.equal(gate, 306);
  assert.equal(booked, 221);
