...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), nineteen sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 19);
  assert.equal(byVerdict("HELD"), 123);
  assert.equal(REPAIR_AUDIT.length, 153);
  ...
  assert.equal(gate, 172);
  assert.equal(booked, 142);
