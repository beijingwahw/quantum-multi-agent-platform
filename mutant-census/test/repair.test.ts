...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), thirty sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 30);
  assert.equal(byVerdict("HELD"), 130);
  assert.equal(REPAIR_AUDIT.length, 171);
  ...
  assert.equal(gate, 232);
  assert.equal(booked, 160);
