...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), sixteen sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 16);
  assert.equal(byVerdict("HELD"), 115);
  assert.equal(REPAIR_AUDIT.length, 142);
  ...
  assert.equal(gate, 163);
  assert.equal(booked, 131);
