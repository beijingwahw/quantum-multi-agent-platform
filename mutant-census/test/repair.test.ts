...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), twenty-five sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 25);
  assert.equal(byVerdict("HELD"), 128);
  assert.equal(REPAIR_AUDIT.length, 164);
  ...
  assert.equal(gate, 222);
  assert.equal(booked, 153);
