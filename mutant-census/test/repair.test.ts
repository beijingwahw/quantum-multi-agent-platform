...
test("the repair state: eleven upgrades (b67#6 and b70#3 flipped under R1), forty-five sharpenings, the rest held", async () => {
  ...
  assert.equal(byVerdict("SHARPENED"), 45);
  assert.equal(byVerdict("HELD"), 161);
  assert.equal(REPAIR_AUDIT.length, 217);
  ...
  assert.equal(gate, 285);
  assert.equal(booked, 206);
