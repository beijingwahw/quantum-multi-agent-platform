...
7. **估计器 + 鲁棒性（v0.2 主交付）**：oracle 换成传感器层——每链路
   ...
   VIOLATION）。
8. **工作守恒不敏感性定理（v0.4.0，`src/net/conservation.ts`）**：饱和
   两链路单请求链上，任何工作守恒策略（每空槽必尝试＋就绪即交换）诱导
   **同一个嵌入随机过程**——四个结构迥异的策略（swap-asap / swap-late /
   tdm / 超订冗余策略）同种子计数器**逐位相同**，平稳交付率全部落在
   精确 DTMC 值的 4σ 内，且等于更新论闭式 **r\* = qSwap / E[max(G₁,G₂)]**
   （闭式与 4 态 DTMC 两独立路恒等到 1e-12；49 态年龄链同理）。
   平稳率是**物理的属性，不是策略的属性**。负对照：奇偶停摆策略
   （空转份额恰为 0——从不浪费尝试槽）损失 ~10% 交付率，其精确值由
   8 态奇偶增广链解出、引擎 4σ 内复认——「损失＝空转份额」恒等式
   在守恒类外被定罪：空转不是损失的唯一来源。多请求面（DATA）：
   六节点三请求网格上 ERS 亏额仲裁的 Jain 公平指数逐种子 ≥ 轮转指针
   （代价：总吞吐更低，如实披露，不主张）。
...
```bash
npm install
npm test          # 73 项测试：物理裁判、引擎记账、策略、Markov 互证、传感器层、工作守恒定理、走私审判与具名错误审判
