...
8. **工作守恒不敏感性定理（v0.4.0，`src/net/conservation.ts`）**：饱和
   ...
   **同一个嵌入随机过程**——四个结构迥异的策略（swap-asap / swap-late /
   ...
   （代价：总吞吐更低，如实披露，不主张）。
9. **纯化梯 IFS 相界定理（v0.5.0，`src/physics/ladder.ts`）**：混合纯化梯
   的经验壁升格为闭式。四层：(L1) Werner 复合 g(f₁,f₂) 等臂不动点集恰为
   **{1/4, 1/2, 1}**（BBPSSW 改进阈值 1/2 的多项式闭式化）；(L2) 新鲜混合
   不动点 **μ(F)** 是显式二次 2(4F−1)μ²+6(1−2F)μ+(F−1)=0 的正根
   （F=0.85 → **0.9093646522**——policies.ts 散文里 "≈0.909" 无操作壁的
   精确化），链梯单调收敛于它；(L3) s 槽稳态饱和保真度满足**嵌套不动点
   递推 S₁=F₀、S_{s+1}=μ(S_s)**，相界 F₀\*(s,fMin)（S_s(F₀)=fMin 的唯一根）
   括号至任意精度——exp4 的经验壁全部成为特例点：S_3(0.85)=0.949369<0.95
   ≤S_4(0.85)=0.973102（**slots 下限 4** ✓）、F₀=0.8 行的 2/3 槽零交付
   （exp4-D 网格列 ✓）、0.9497 饱和墙=S_3+MIN_GAIN 截断；(L4) 一次性平衡
   批次（"2^k 阶梯"）B_{k+1}=g(B_k,B_k)：F₀=0.84/0.85 到 0.95 恰需
   **2⁴=16 叶**（exp4-A 的 "the 2^4 ladder" ✓）。引擎交叉核对：(F₀,slots)
   网格上交付⟺S_slots≥fMin（种子化 3 种子×30k 轮）；负对照双枚——
   s−1 槽绕壁断言被引擎零交付定罪、伪造相界值（0.75）被两侧无符号翻转
   定罪。文献：BBPSSW-1996（仓内双源）；Hutchinson-1981 迭代函数系统
   〔待双源〕。诚实边界：Werner/twirl 后族、值域分析（成功率与时间动力学
   留给引擎）。
...
```bash
npm install
npm test          # 94 项测试：物理裁判、引擎记账、策略、Markov 互证、传感器层、工作守恒定理、纯化梯 IFS 相界定理、走私审判与具名错误审判
